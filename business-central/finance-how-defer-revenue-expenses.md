...
description: 'Узнайте ,как автоматически отнести на РБП или отложить доходы и расходы в периодах, когда транзакция не была учтена, или отложить их по указанному графику.'
author: brentholtorf
ms.author: bholtorf
...
Для выявления дохода или расхода в периоды, отличные от периода, в котором транзакция была учтена, можно автоматически отнести на РБП доходы и расходы по указанному графику.
...
> [!NOTE]
> Журналы продаж и покупок подтверждают исходный код. Для проверки необходимо, чтобы исходный код для продаж и журналов продаж, а также для покупок и журналов покупок соответственно, не был идентичен при использовании отсрочек. Если он настроен идентичным, вы можете обойти это ограничение, создав шаблон и пакет, использующие другой исходный код.

## <a name="to-set-up-a-gl-account-for-deferral"></a>Настройка счета ГК для РБП

1. Выберите значок ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок, введите **План счетов**, а затем выберите связанную ссылку.
...
4. В поле **Метод расчета** укажите, как рассчитывается поле **Сумма** для каждого периода на странице **Расписание РБП**. Можно выбирать из следующих параметров:
   ...
   * **Равномерно на период**: периодические суммы отсрочки рассчитываются в соответствии с количеством периодов и равномерно распределяются по периодам.
   * **Дней за период**: суммы периодической отсрочки рассчитываются в зависимости от количества дней в периоде.
   * **Определяется пользователем**: периодические суммы отсрочки не рассчитываются. Необходимо вручную ввести значения в поле **Сумма** за каждый период на странице "Расписание РБП". Дополнительные сведения см. в разделе "Изменение расписания РБП из счета продажи".
...
3. На вкладке **Затраты и проводки**  в поле **Шаблон отсрочки по умолчанию**  выберите соответствующий шаблон отсрочки.
...
## <a name="to-specify-a-period-in-which-to-allow-deferral-posting"></a>Задание периода, в котором разрешен учет расходов/доходов будущих периодов
