...
Прежде чем вы сможете начать использовать функциональность, необходимо выполнить определенные настройки. Дополнительные сведения см. также в разделе [Настройка [!INCLUDE[d365fin](includes/d365fin_md.md)]](setup.md). Обратите внимание, что некоторые этапы настройки можно выполнить полуавтоматически с помощью мастеров настройки.
...
Также на сайте [Microsoft Learn](/learn/browse/?products=dynamics-business-central) есть электронные учебные материалы.  
...
| Настройка соединителя Business Central |Настройка подключения к Dynamics 365 Sales, которое позволяет синхронизировать данные, такие как контакты и заказы на продажу. |
