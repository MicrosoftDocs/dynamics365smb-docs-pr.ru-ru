...
> [!IMPORTANT]
> Базовый модуль «Электронные документы» представляет собой платформу. По умолчанию поля **Формат документов** или **Интеграция службы** нет. Эти сведения входят в состав приложений локализации, поскольку оба они зависят от местных требований.

> [!NOTE]
> Начиная с версии 23.1 стандартный формат документа PEPPOL добавлен в качестве глобального формата в поле **Формат документов**.
...
## Настройка службы электронных документов
...
2. Выберите **Создать**, а затем на странице **Служба электронных документов** на экспресс-вкладке **Общие сведения** настройте поля, как описано в следующей таблице.

    | Поле | Описанием |
    |-------|-------------|
    | Код | Выберите код настройки электронного экспорта. |
    | Описанием | Введите краткое описание настройки электронного экспорта. |
    | Формат документов | <p>Формат экспорта для настройки электронного экспорта.</p><p>По умолчанию в волне 1 в этом поле нет вариантов значений.</p> |
...
4. На экспресс-вкладке **Параметры импорта** заполните поля, как описано в следующей таблице.
...
Если вы настроили формат **Определение обмена данными** в своей локализации, вы можете добавить по строке для каждого типа документа, который вам нужен. Однако сначала необходимо выбрать значение параметра **Тип документа** для каждой необходимой вам строки. Для каждого типа данных выберите **Код определения обмена данными (импорт)** или **Код определения обмена данными (экспорт)**, который вы хотите использовать.

В конце концов, если вы не используете формат **Определение обмена данными**, вы можете настроить форматы с помощью строк **Сопоставление экспорта** и **Сопоставление импорта**, где можно указать таблицы и поля для использования и настроить правила преобразования, если необходимо.

## Настройка профиля отправки документов
...
4. Запустите действие **Создать рабочий процесс из шаблона**, чтобы выбрать шаблон для процесса обработки электронных документов. Доступные шаблоны — **Отправка в одну службу** и **Отправка в несколько служб**.
...
7. Выберите созданную вами службу электронных документов, нажмите **ОК**, а затем включите рабочий процесс.
...
## Настройка политики хранения электронных документов
...
1. На странице **Службы электронных документов** запустите действие **Политика хранения**.
