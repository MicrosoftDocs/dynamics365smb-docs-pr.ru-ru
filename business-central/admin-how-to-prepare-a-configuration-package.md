---
title: Подготовка пакета конфигурации
description: Узнайте сейчас, чтобы подготовить пакет конфигурации RapidStart, который может помочь настроить новые компании на основе существующих данных.
author: brentholtorf
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
8. Нажмите кнопку **ОК**. Число выбранных таблиц отображается в поле **Выбрать таблицы**. Укажите дополнительные параметры, а затем нажмите кнопку **ОК**. Таблицы [!INCLUDE[prod_short](includes/prod_short.md)] будут добавлены в строки страницы **Пакет конфигурации**.  
...
10. При желании, чтобы применить фильтры обработки к данным таблицы или добавить кодовую единицу codeunit с любым кодом, который вы хотите включить в пакет, выберите строку для соответствующей таблицы, затем выберите действие **Правила обработки**.

    1. На странице **Правила обработки таблицы конфигурации** заполните поля. [!INCLUDE [tooltip-inline-tip_md](includes/tooltip-inline-tip_md.md)]

        - Чтобы применить фильтры к данным, укажите соответствующее действие в поле **Действие**, выберите действие **Фильтры обработки**, затем заполните поля.  

            Например, пакеты конфигурации Microsoft для оценочных компаний устанавливают фильтры обработки на таблицы **Заголовок продаж** и **Заголовок покупки**.
        - Чтобы добавить модуль обработки codeunit, укажите его в поле **Пользовательская обработка ИД Codeunit**.

          > [!NOTE]
          > Этот модуль codeunit должен принимать таблицу 8614 *Запись пакета конфиг.* как параметр метода `OnRun`.
    2. Закройте страницу.
11. Определите, не были ли внесены потенциальные ошибки, выбрав действие **Проверить пакет**. Это может произойти, если пользователь не включает таблицы, от которых зависит его конфигурация.  
...
2. В журнале конфигураций определите таблицы, данные которых необходимо просмотреть или изменить.  
    ...
    > [!NOTE]  
    >  Убедитесь, что каждая таблица имеет код страницы, присвоенный ей. Для стандартных таблиц [!INCLUDE[prod_short](includes/prod_short.md)] это значение заполняется автоматически. В случае пользовательских таблиц необходимо указать код.
...
[Анализ телеметрии трассировки пакета конфигурации](/dynamics365/business-central/dev-itpro/administration/telemetry-configuration-package-trace)  
