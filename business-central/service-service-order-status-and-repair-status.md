...
ms.author: edupont
---
# <a name="service-order-status-and-repair-status" />Статус сервисного заказа и ремонта
...
## <a name="specifying-service-order-status-for-repair-status" />Указание статуса сервисного заказа для определения статуса ремонта
...
### <a name="finished" />Завершен

Статус сервисного заказа **Завершен** указывает, что сервис выполнен. Поэтому статус ремонта **Завершен** связан с этим статусом.  

## <a name="assigning-priority-to-service-order-status" />Назначение приоритета со статусом сервисного заказа
