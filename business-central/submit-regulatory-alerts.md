...
4. Вы можете добавить новое оповещение, нажав знак плюс **(+)** в верхней части списка.
    ...
    Откроется 4-шаговый мастер для создания оповещения. Руководство содержит следующие шаги:
    - Поиск существующих элементов.

        Выполните поиск информации, могущей иметь отношение к оповещению, которое вы планируете создать. Если вы не нашли никаких подходящих результатов, можно выбрать кнопку **Отправить нормативное оповещение** внизу страницы, чтобы продолжить отправку оповещения.
    - Прикрепление бизнес-процессов.

        Эта часть не относится к Dynamics 365 Business Central. Выберите **Пропустить**, чтобы перейти к следующему шагу.
    - Описание оповещения.

        Введите информацию об оповещении в соответствующие поля. Красная звездочка (\*) в мастере означает, что поле является обязательным.

        |Поле        |Описанием                               |
        |-------------|------------------------------------------|
        |Должность  | Введите описательный заголовок для определения области влияния. Введите, например, *Изменения в документах по счетам от 1-го июля 2019*. |
        ...
        |Ссылка на компонент  | Это не относится к Business Central, вы можете указать ссылку на компонент, если он вам известен. Список компонентов для определенной страны/региона можно найти на [Портале локализации](/dynamics/s-e/) на сайте CustomerSource. |
        ...
        |Комментарии  | Введите любую дополнительную информацию, которая может помочь пользователям понять или реализовать требование. Выберите **Отправить**, чтобы сохранить комментарий. Несколько комментариев можно добавлять, и они должны отправляться отдельно. Комментарии сохраняются в порядке их добавления. |
        |Вложения  | Выберите кнопку **Отправить**, затем найдите и выберите файл, чтобы добавить его как вложение. После выбора файла он отправляется в систему и появляется в качестве прикрепленного файла. Можно добавить до трех файлов размером 5 МБ каждый. Чтобы удалить вложенные файлы, выберите **Удалить** под заголовком файла. Вложения должны быть публично доступными материалами. Они не могут объектом чьей-либо интеллектуальной собственности или относиться к конкретному клиенту или партнеру.|

        Выберите **Отправить**, чтобы сохранить и отправить оповещение.
