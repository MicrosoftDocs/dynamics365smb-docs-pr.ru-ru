...
> [!Tip]
> После выполнения шагов в руководстве вы можете проверить, было ли соединение успешным. Выполните поиск **Настройка модуля "Маркетинг"**, выберите **Доступ**, затем **Функции**, потом **Проверить настройку регистрации эл. почты**.
...
- Превратите обмен электронной почтой в возможность продажи — если запись выглядит многообещающей, вы можете превратить ее в возможность, а затем управлять ее продвижением к продаже. Для этого выберите запись, а затем выберите **Обработка**, а потом **Создать возможность**. Дополнительные сведения см. в разделе [Управление возможностями продаж](marketing-manage-sales-opportunities.md).
...
Чтобы подключиться к Exchange Online, вы должны использовать **OAuth2** как **Тип аутентификации**. Вы также должны зарегистрировать приложение в Azure Active Directory и указать идентификатор приложения, секретный ключ хранилища и используемый URL-адрес перенаправления. URL-адрес перенаправления предварительно заполнен и должен работать для большинства установок. Дополнительные сведения см. в разделе Регистрация приложения в Azure AD для подключения из Business Central к Exchange Online ниже.
