...
---
# <a name="set-up-currencies"></a>Настройка валют

[!INCLUDE [finance-currencies-def](includes/finance-currencies-def.md)]

Используйте внешнюю службу для получения самых актуальных валютных курсов в списке **Валюты**. Дополнительные сведения см. в разделе [Настройка служб валютных курсов](finance-how-update-currencies.md#set-up-a-currency-exchange-rate-service).  

[!INCLUDE [finance-currencies-lcy](includes/finance-currencies-lcy-note.md)]

## <a name="currencies"></a><a name="curr"></a>Валюты
...
|**Счет реализованной прибыли**|Счет, на котором учитывается фактическая прибыль при получении платежей по дебиторской задолженности или регистрации фактического курса валюты по платежам в счет кредиторской задолженности. Для примера валютной проводки дебиторской задолженности см. пример под этой таблицей. |
|**Счет реализованного убытка**|Счет, на котором учитывается фактический убыток при получении платежей по дебиторской задолженности или регистрации фактического курса валюты по платежам в счет кредиторской задолженности. Для примера валютной проводки дебиторской задолженности см. пример под этой таблицей. |
|**Счет нереализованной прибыли**|Счет, на котором учитывается теоретическая прибыль при коррекции валюты.|
|**Нереализованный убыток**|Счет, на котором учитывается теоретический убыток при коррекции валюты.|
|**Точность округления суммы**|Некоторые валюты имеют другие форматы для сумм счетов, чем те, которые определены на странице **Настройка ГК**. Если вы измените точность округления сумм для валюты, все суммы счетов в этой валюте будут округлены в соответствии с новой точностью.|
|**Десятичные знаки в сумме**|Некоторые валюты имеют другие форматы для сумм счетов, чем те, которые определены на странице **Настройка ГК**. Если вы измените десятичные знаки в сумме для валюты, все суммы счетов в этой валюте будут округлены в соответствии с новым количеством десятичных знаков|
...
|**Точность округления цены единицы**|Некоторые валюты имеют другие форматы для сумм единицы, чем те, которые определены на странице **Настройка ГК**. Если вы измените точность округления цен за единицу для валюты, все цены за единицу в этой валюте будут округлены в соответствии с новой точностью.|
|**Десятичные знаки в цене единицы**|Некоторые валюты имеют другие форматы для сумм единицы, чем те, которые определены на странице **Настройка ГК**. Если вы измените десятичные знаки в цене за единицу для валюты, все цены за единицу в этой валюте будут округлены в соответствии с новым количеством десятичных знаков|
...
||**Коррекция Курс. Разниц**|Обновление сальдо для операций в главной книге, книгах поставщиков, клиентов и банковских счетов. Это обновление полезно, если курс валюты изменился после учета операций.|
...
## <a name="lcy-and-other-currencies"></a>Местная и другие валюты

[!INCLUDE [finance-currencies-lcy-def](includes/finance-currencies-lcy-def.md)]

## <a name="rounding-currencies"></a>Валюты для округления
...
В отличии от функций округления счетов функции округления сумм и цен влияют только на суммы в иностранной валюте, а не на соответствующие суммы в рублях. При их использовании на счетах главной книги ничего не учитывается. Соответственно, счета главной книги можно не указывать ни в учетных группах, ни где бы то ни было.

### <a name="unit-amount-rounding"></a>Округление цены
...
### <a name="amount-rounding"></a>Округление суммы
...
## <a name="general-ledger-accounts"></a>Счета ГК

Коды валют нельзя связывать со счетами главной книги, поскольку суммы на счетах главной книги выражены в МВ. При наличии банковской ссуды в долларах США и зачислении депозитов на банковский счет в шведских кронах эти счета можно отслеживать, указав в настройках банковских счетов в доллары США и шведские кроны. Учетные группы используются для того, чтобы связать банковские счета с соответствующими счетами главной книги. В Главной книге величина сумм отражается в рублях.

В строке финансового журнала можно ввести код валюты и учесть ее в счете главной книги. Для конвертации суммы в МВ перед ее учетом на счете главной книги используется соответствующий валютный курс.  
