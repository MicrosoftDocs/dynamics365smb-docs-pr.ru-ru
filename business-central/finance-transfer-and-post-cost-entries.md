---
title: Передача и размещение записей затрат
...
# <a name="transferring-and-posting-cost-entries"></a>Передача и размещение записей затрат
...
## <a name="criteria-for-transferring-general-ledger-entries-to-cost-entries"></a>Критерии перевода записей главная книга в записи затрат

Важно понимать критерии перевода записей главная книга в записи затрат. Во время перемещения пакетное задание **Перенести записи ГК в CA** использует следующие критерии, чтобы определить, перемещаются ли операции главной книги и если да, то как.  
...
Записи главная книга не переносятся, если:  

- Записи имеют значения измерений, которые не соответствуют центру затрат или объекту затрат.  
- Операции имеют нулевую сумму.  
- У операций имеется счет главной книги, который был удален.  
- Записи содержат учетную запись главная книга, которая не относится к типу **Отчет о прибылях и убытках**.  
- Записи содержат учетную запись главная книга, которой не назначен тип затрат.  
...
## <a name="transferring-general-ledger-entries-to-cost-entries"></a>Перенос записей главная книга в записи затрат
...
2.  Нажмите кнопку **Да** для запуска перемещения. Процесс переносит все записи главная книга, которые еще не были перенесены.  
...
## <a name="automatic-transfer-and-combined-entries"></a>Автоматический перенос и комбинированные записи
...
## <a name="results-of-transferring-general-ledger-entries-to-cost-entries"></a>Результаты перевода записей главная книга в записи затрат
...
### <a name="cost-register"></a>Регистр расходов

В таблице **Регистр затрат** [!INCLUDE[prod_short](includes/prod_short.md)] создает запись с перемещением источника из Главной книги. В записи регистрируются первый и последний номера записей главная книга, которые переносятся, в дополнение к первому и последнему номерам записей созданных записей затрат.
