...
---
# <a name="set-up-data-exchange-definitions"></a>Создание определений обмена данными
...
Обычно определения обмена данными настраиваются на странице **Определение обмена данными**. Однако для обновления валютных курсов быстрее будет использовать службу валютных курсов. Подробнее см. в статье [Обновление валютных курсов](finance-how-update-currencies.md#set-up-a-currency-exchange-rate-service).
...
## <a name="create-a-data-exchange-definition"></a>Создание определения обмена данными
...
### <a name="to-describe-the-formatting-of-lines-and-columns-in-the-file"></a><a name=formatlinescolumns></a>Описание форматирования строк и столбцов в файле
...
3. На экспресс-вкладке **Общее** введите описание определения обмена данными и тип файла данных, заполнив поля, как описано в следующей таблице.  
    ...
    |**Чтение/запись XMLport**|Укажите XMLport, через который принимаются импортируемый файл или служба данных перед сопоставлением и через который экспортируемые данные записываются в файл или службу данных после сопоставления.|  
...
5. Повторите шаг 4, чтобы создать строку для каждого типа файла данных, который необходимо экспортировать.  
     ...
     Перейдите к описанию форматирования столбцов файла данных, заполнив поля экспресс-вкладки **Определения столбцов** как описано в таблице в шаге 8. Можно использовать файл структуры — например, формата XSD — в качестве файла данных для заполнения экспресс-вкладки соответствующими элементами. Подробнее см. в статье [Использование XML-схем для определения обмена данными](across-how-to-use-xml-schemas-to-prepare-data-exchange-definitions.md).
...
8. На экспресс-вкладке **Определения столбца** измените или заполните поля, как описано в следующей таблице.  
    ...
    |**Языковой стандарт форматирования данных**|Укажите региональный формат данных, если он есть. Например, **en-US**, если используется тип данных **Десятичный**, чтобы гарантировать, что запятая используется в качестве разделителя 0.000, в соответствии с форматом США. Подробнее см. в статье [Стандартные строки форматирования даты и времени](/dotnet/standard/base-types/standard-date-and-time-format-strings). **Примечание.** Это поле релевантно только для импорта.|  
    ...
    |**Идентификатор отрицательного знака**|Введите значение, используемое в файле данных для определения отрицательных сумм в файлах данных, которые не могут содержать отрицательные знаки. Затем этот идентификатор используется для добавления отрицательных знаков к идентифицированным суммам во время импорта. **Примечание.** Это поле релевантно только для импорта.|  
...
### <a name="to-map-columns-in-the-data-file-to-fields-in-"></a><a name=mapfields></a>Сопоставление столбцов в файле данных с полями в [!INCLUDE[prod_short](includes/prod_short.md)]
...
2. На экспресс-вкладке **Общее** укажите настройку сопоставления, заполнив поля, как описано в следующей таблице.  
    ...
    |**Использовать как промежуточную таблицу**|Укажите, является ли таблица, выбранная в поле **Код таблицы**, промежуточной таблицей, в которой импортируемые данные сохраняются перед сопоставлением с целевой таблицей.<br/><br/> Обычно промежуточная таблица используется, когда определение обмена данными применяется для импорта и преобразования электронных документов в [!INCLUDE[prod_short](includes/prod_short.md)]. Например, это может быть преобразование счетов поставщиков в счета покупки. Подробнее см. в статье [Электронный обмен данными](across-data-exchange.md).|  
    |**Имя**|Ввод имени настройки сопоставления.|  
...
3. На экспресс-вкладке **Сопоставление полей** укажите, какие столбцы сопоставляются с какими полями в [!INCLUDE[prod_short](includes/prod_short.md)], заполнив поля, как описано в следующих таблицах, в зависимости от того, включено поле **Использовать как промежуточную таблицу** или нет.  
   * Если переключатель **Использовать как промежуточную таблицу** находится в положении "выключено":
     ...
     |**ИД поля**|Укажите поле, с которым требуется сопоставить столбец в поле **Номер столбца** .<br /><br /> Можно выбирать только из полей, которые существуют в таблице, указанной в поле **ИД таблицы** на экспресс-вкладке **Общие сведения**.|
     ...
     |**Возможно**|Укажите, следует ли пропустить сопоставление, если поле не заполнено. Если этот флажок не установлен, произойдет ошибка экспорта, если поле пустое.|  
     |**Правило преобразования**|Укажите правило для преобразования импортированного текста в поддерживаемое значение, прежде чем это значение можно будет сопоставить с указанным полем. Когда вы выбираете значение в этом поле, это же значение вводится в поле **Правило трансформации** в таблице **Буфер сопоставления полей обмена данными** и наоборот. Подробнее о том, как применять правила преобразования, см. в статье [Правила преобразования](#transformation-rules).|
     |**Перезаписать значение**|Укажите, что текущее значение может быть перезаписано новым значением.|
     |**Приоритет**|Укажите порядок, в котором должны обрабатываться сопоставления полей. Сопоставление полей с наивысшим номером приоритета будет обрабатываться в первую очередь.|
   ...
   * Если переключатель **Использовать как промежуточную таблицу** находится в положении "включено":
     ...
     |**Заголовок таблицы**|Укажите имя таблицы в поле **Код целевой таблицы**, которое задает таблицу, с которой сопоставляется значение в поле **Заголовок столбца**, при использовании промежуточной таблицы для импорта данных.|
     ...
     |**Подпись поля**|Укажите имя поля в целевой таблице, с которым сопоставляется значение в поле **Заголовок столбца**, при использовании промежуточной таблицы для импорта данных.|
     ...
     |**Правило преобразования**|Укажите правило для преобразования импортированного текста в поддерживаемое значение, прежде чем это значение можно будет сопоставить с указанным полем. Когда вы выбираете значение в этом поле, это же значение вводится в поле **Правило преобразования** в таблице **Буфер сопоставления полей обмена данными** и наоборот. Подробнее о правилах преобразования см. в статье [Правила преобразования](#transformation-rules).|
     |**Приоритет**|Укажите порядок, в котором должны обрабатываться сопоставления полей. Сопоставление полей с наивысшим номером приоритета будет обрабатываться в первую очередь.|
...
4. На экспресс-вкладке **Группирование полей** укажите правила, которые вы хотите использовать для группирования полей при создании файла, заполнив поля, как описано в следующей таблице.  
     ...
     |Поле|Описанием|  
     |--------------------------------- |---------------------------------------|  
     |**ИД поля**|Укажите номер поля во внешнем файле, которое используется для группирования, и что пользователь должен задать это поле.|
     |**Подпись поля**|Укажите метку поля во внешнем файле, которое используется для группирования.|
...
|**Удалить отличные от буквенно-цифровых символы**|Удалить символы, которые не являются буквами или цифрами, например знаки препинания или специальные символы.|
...
|**Пользовательский**|Это правило преобразование представляет собой расширенный вариант, для использования которого потребуется помощь разработчика. Он позволяет использовать событие интеграции, на которое вы можете подписаться, если хотите использовать свой собственный код преобразования. Если вы разработчик и хотите использовать эту опцию, см. раздел [Совет для разработчиков: пример пользовательской опции](#tip-for-developers-example-of-the-custom-option).|
...
## <a name="export-a-data-exchange-definition-as-an-xml-file-for-use-by-others"></a>Экспорт определения обмена данными в XML-файл для использования другими пользователями

После того как вы создали определение обмена данными для конкретного файла данных, можно экспортировать его в виде XML-файла для последующего импорта. Эта задача описана в следующей процедуре.  
...
## <a name="import-an-existing-data-exchange-definition"></a>Импорт существующего определения обмена данными
