---
title: 'Технические подробности: обработка политик дозаказа'
...
---
# <a name="design-details-handling-reordering-policies"></a>Технические подробности: обработка политик дозаказа
...
## <a name="the-role-of-the-reorder-point"></a>Роль точки повтора заказа

Точка дозаказа представляет спрос во время подготовки. Когда прогнозируется, что запасы опустятся ниже уровня, определенного точкой повторного заказа, время заказывать еще. Запасы постепенно уменьшаются, пока не прибудет пополнение. Он может достичь нуля или уровня страхового запаса. Система планирования предложит заранее планируемый заказ на поставку в момент перехода запасов ниже точки повтора заказа.  
...
## <a name="monitoring-the-projected-inventory-level-and-the-reorder-point"></a>Отслеживание уровня прогнозируемых запасов и точки повтора заказа
...
### <a name="projected-inventory"></a>Прогнозируемые запасы
...
* Если событие спроса полностью удовлетворено, прогнозируемый расход склада не осуществляется сразу же. Вместо этого разносится напоминание об уменьшении, которое является внутренней записью, содержащей дату и количество добавления в прогнозируемые запасы.  
...
* При появлении нового заказа на поставку она проверяет, был ли он введен до текущей поставки. В этом случае новая поставка становится текущей поставкой, и процедура балансировки начинается снова.  
...
7. Система планирования делает окончательную проверку. Есть ли напоминание об уменьшении? Да, имеется, в дату **Da**.  
...
Назначение горизонта планирования — сбор событий требований в пределах временного периода с целью составления общего заказа на поставку.  
...
## <a name="stay-below-the-overflow-level"></a>Оставайтесь ниже уровня допустимого избытка
...
### <a name="creating-the-planning-line-with-an-overflow-warning"></a>Создание строки планирования с предупреждением о переполнении

Строка планирования создается, когда из-за существующей поставки прогнозируемые запасы выше допустимого избытка по окончании горизонта планирования. Чтобы предупредить о возможных избыточных поставках, строка планирования содержит предупреждение, поле **Принять указание** не выбрано, а указание имеет значение **Отменить** или **Изменить кол-во**.  
...
#### <a name="situation-before-a-sales-decrease"></a>Ситуация до снижения продаж

|Событию|Изменение кол-ва|Ожидаемые запасы|  
|-----------|-----------------|-------------------------|  
|День первый|Нет|80|  
|Продажа|-70|10|  
...
Система планирования считает чрезвычайной ситуацией, если будущий спрос не может быть обслужен за счет прогнозируемого запаса. Или, выражаясь по-другому, прогнозируемый запас становится отрицательным. Система предлагает создать новый заказ на поставку для покрытия неудовлетворенной части спроса. Размер нового заказа на поставку не учитывает максимальный запас или количество дозаказа, а также следующие модификаторы заказа:
...
В строке планирования для этого заказа на поставку отображается предупреждающий значок **Экстренный**для просмотра дополнительных сведений о ситуации.  
...
3. Поставка **A** закрыта спросом **B**, который не удовлетворен в полном размере.  
...
7. Прогнозируемые запасы проверены. Точка дозаказа не пройдена.  
...
### <a name="fixed-reorder-quantity"></a>Фиксированное кол-во для дозаказа
...
Система не будет предлагать новый заказ на поставку, если поставка доведет запланированные запасы до точки дозаказа в течение времени подготовки.  

Заказы на поставку, созданные специально для достижения точки дозаказа, исключаются из балансировки поставки и не меняются. Если вы хотите поэтапно отказаться от товара, для которого есть точка повторного заказа, просмотрите незавершенные заказы на поставку вручную или измените политику повторного заказа на **Партия на партию**. Система уменьшает или отменяет избыточную поставку.  
...
Прежде чем предложить новый заказ на поставку для достижения точки дозаказа, система планирования проверяет, не запланирован ли заказ на нерабочий день. Она использует календари, указанные вами в поле **Код базового календаря** на страницах **Информация о компании** и **Карточка склада**.  

Если плановая дата является нерабочим днем, система планирования сдвинет заказ вперед до ближайшего рабочего дня. Смещение даты может привести к тому, что заказ достигнет точки повтора заказа, но не удовлетворит определенному спросу. Если такого несбалансированного спроса система планирования создает дополнительную поставку.  

#### <a name="shouldnt-be-used-with-forecasts"></a>Не для использования с прогнозами
...
#### <a name="combine-with-order-modifiers"></a>Объедините с модификаторами заказа

В зависимости от вашей настройки может быть лучше объединить политику максимального количества с модификаторами заказа:

* Чтобы обеспечить минимальное количество заказа
...
### <a name="combine-with-calendars"></a>Объедините с календарями
...
В некотором смысле политика "партия на партию" похожа на политику "заказ". Она может принимать количества в запасах и связывать спрос и предложение в определенные вами горизонты планирования.
...
Горизонт планирования также устанавливает лимиты перепланирования заказа на поставку с целью удовлетворения имеющегося спроса. Поставка в пределах горизонта планирования будет перепланирована на более раннюю или позднюю дату, чтобы удовлетворить спрос. Более ранняя поставка приведет к избыточным запасам, и вы должны отменить ее. Для поставки позже создайте новый заказ на поставку.  

С помощью этой политики вы можете указать страховой запас, чтобы компенсировать изменения в поставке или удовлетворить внезапный спрос. Политика " "партия на партию" может также включать буферный период и буферное количество, чтобы уменьшить объем работы по планированию заказов.  

Вместе с полем **Период перепланирования** поле **Период накопления лота** играет роль в определении цикла дозаказа. Начиная с даты первого спроса система накапливает весь спрос в следующем периоде накопления лота в один заказ на поставку на дату первого спроса. Спрос за пределами периода накопления лота не покрывается этим заказом на поставку.
