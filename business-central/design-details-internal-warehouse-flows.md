---
title: 'Сведения о проектировании — потоки для производства, сборки и проектов'
...
|---|----------------|----------|---------|------------------|------------------|------------------|
|Нет отдельной складской операции.|Учет из заказов и журналов.||Необязательно. Управляется переключателем **Код ячейки обязателен**.|Производственный журнал -> Выходной журнал</br><br/> **ПРИМЕЧАНИЕ**. Вы можете разнести выходные данные, используя **Производственный журнал**.|Заказ на сборку|Размещение не применимо для проектов|  
|Базовая|Последовательно по каждому заказу.|Требуется размещение. </br><br/> **ПРИМЕЧАНИЕ**. Хотя этот параметр называется **Требуется размещение**, все равно можно учитывать выход из исходных документов на складах, для которого установлен этот флажок. |Необязательно. Управляется переключателем **Код ячейки обязателен**.|Производственный заказ -> Размещение запасов|Заказ на сборку|Размещение не применимо для проектов|
|Расширенное|Консолидированные действия по размещению для нескольких исходных документов.|Требуется приемка + Требуется размещение|Необязательно. Управляется переключателем **Код ячейки обязателен**.|Производственный заказ(ы) -> Журнал выхода продукции|Заказы на сборку -> внутренние перемещения | Размещение не применимо для проектов|
|Расширенное|То же, что и выше + деятельность расширенного подбора и размещения|Расширенный подбор и размещение (зависимые переключатели будут включены автоматически)|Обязательно|То же, что выше|То же, что выше| Размещение не применимо для проектов|
...
|Уровень сложности|Описание|Параметры|Код ячейки|Исходящий поток производственного заказа|Исходящий поток заказа на сборку|Исходящий поток проектов|  
|---|----------------|----------|---------|------------------|------------------|------------------|
...
|Расширенное|Консолидированные действия по подбору для нескольких исходных документов.|Требуется отгрузка + Требуется подбор|Необязательно. Управляется переключателем Код ячейки обязателен|Производственный заказ -> Складской подбор -> Журнал потребления |Заказы на сборку -> Складской подбор| Проекты –> Складской подбор -> Журнал проектов |
|Расширенное|То же, что и выше + деятельность расширенного подбора и размещения|Расширенный подбор и размещение (зависимые переключатели будут включены автоматически)|Обязательно|То же, что выше|То же, что выше| Расширенный подбор и размещение не поддерживается для проектов|
