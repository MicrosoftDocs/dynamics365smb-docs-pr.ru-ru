...
Финансовые отчеты в стандартной версии [!INCLUDE[prod_short](includes/prod_short.md)] лежат в основе стандартных финансовых отчетов, которые могут не соответствовать потребностям вашей компании. Чтобы быстро создать собственный финансовый отчет, можно сначала скопировать имеющийся отчет. См. ниже шаг 3.
...
Используйте раскладку столбцов для задания столбцов, включаемых в получающийся отчет. Например, можно создать структуру для сопоставления оборота и баланса за одинаковый период текущего года и прошлого года. Вы можете иметь до 15 столбцов. Это полезно, например, если вы хотите просмотреть бюджеты за 12 месяцев и включить столбец, показывающий итоги.
...
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать 2.](media/ui-search/search_small.png "Что вы хотите сделать") значок, введите **Финансовые отчеты**, а затем выберите связанную ссылку.
...
[!INCLUDE[prod_short](includes/prod_short.md)] использует формулу периода для расчета суммы из периода сравнения по отношению к периоду, представленному в фильтре дат запроса отчета. Период сравнения основан на периоде начальной даты из фильтра дат. Для обозначений периодов используются следующие сокращения.
...
Вы можете импортировать и экспортировать финансовые отчеты как пакеты конфигурации RapidStart. Например, это полезно для обмена ими с другими компаниями. Пакет создан в файле .rapidstart, который передает содержимое пакета в сжатом формате.
