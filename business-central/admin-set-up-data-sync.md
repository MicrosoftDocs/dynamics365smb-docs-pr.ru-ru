...
Если у вас есть две или более компаний, которые используют хотя бы часть одинаковых основных данных, вы можете сэкономить время на вводе данных, синхронизируя данные в компаниях. Синхронизация данных особенно эффективна при создании новых дочерних компаний.

Основные данные включают настройки и нетранзакционную информацию о бизнес-объектах, таких как клиенты, поставщики, товары и сотрудники. Данные обеспечивают контекст для бизнес-транзакций. Ниже приведены несколько примеров основных данных для клиента:
...
Чтобы можно было настроить синхронизацию, должны выполняться приведенные ниже требования.

* Все компании должны находиться в одной и той же среде.
* Пользователь, настраивающий дочернюю компанию, должен обладать набором разрешений **Упр. основными данными — Просмотр**. Этот набор разрешений доступен в лицензиях Premium и Essential. Лицензия Team Member позволяет пользователям получать доступ к записям, но не изменять их, поэтому ее нельзя использовать для настройки синхронизации.
...
Чтобы сэкономить ваше время, [!INCLUDE [prod_short](includes/prod_short.md)] предоставляет список таблиц, которые часто синхронизируются предприятиями. Синхронизация этих таблиц по умолчанию включена, но вы можете изменять, отключать или удалять их по своему усмотрению. Для дополнительной экономии времени в таблицах уже отключены некоторые поля, которые, предположительно, не имеют отношения к дочерней компании.
...
Вы можете указать данные для синхронизации для таблицы путем определения совпадения записей на основе критериев. На странице **Настройка управления основными данными** выберите действие **Связывание на основе совпадений**. Откроется страница **Выбор критериев связывания**. Вы можете определить следующие критерии для определения совпадения:
...
* Если у вас уже есть записи как в исходной, так и в дочерней компаниях и вы хотите определить совпадение существующих записей, выберите действие **Использовать связывание на основе совпадений**. [!INCLUDE [prod_short](includes/prod_short.md)] сравнит записи в дочерней компании с записями в исходной компании на основе заданных вами критериев определения совпадения. Для нескольких таблиц по умолчанию в [!INCLUDE [prod_short](includes/prod_short.md)] уже определены совпадения между существующими записями с использованием их первичного ключа, но вы можете изменить это, если хотите. Вы также можете разрешить процессу синхронизации создавать новые записи в дочерней компании, соответствующие записям в исходной компании, которых нет в дочерней компании. Чтобы узнать больше о связывании на основе совпадений, перейдите к разделу [Использование связывания на основе совпадений](#use-match-based-coupling).
* Если вы выберете **Запустить полную синхронизацию**, при синхронизации будут создаваться новые записи для всех еще не связанных записей в исходной компании. Как правило, этот параметр полезен, если у дочерней компании еще нет никаких данных в таблице или если вы просто хотите добавить записи из исходной компании без определения совпадений.  
