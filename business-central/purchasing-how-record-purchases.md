---
title: Регистрация покупок с помощью счетов покупки (содержит видео)
description: 'Описывается, как закупать запасы, не учитываемые в запасах товары или ресурсы путем создания и учета счетов покупки или заказов на покупку.'
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
### <a name="create-and-post-a-purchase-invoice"></a>Создание и учет счета покупки
...
Можно легко скорректировать или отменить учтенный счет покупки до оплаты поставщику. Например, это можно сделать, если необходимо исправить опечатку или внести изменение в покупку на раннем этапе обработки заказа. Подробнее в разделе [Исправление или отмена неоплаченных счетов покупки](purchasing-how-correct-cancel-unpaid-purchase-invoices.md). Чтобы сторнировать покупку товаров или услуг в учтенном счете покупки, платеж по которому обработан, необходимо создать кредит-ноту покупки. Подробнее в разделе [Обработка возвратов покупки или отмен](purchasing-how-process-purchase-returns-cancellations.md).
...
## <a name="receive-more-items-than-you-ordered"></a>Получение большего количества товаров, чем заказано
...
В следующем видео показано, как работать с переприходами.
...
Обрабатывая переприход, вы можете увеличить значение в поле **Кол-во к получению** до фактически полученного количества. Поле **Количество переприхода** затем обновляется, чтобы показать избыточное количество. Кроме того, вы можете ввести избыточное количество в поле **Количество переприхода**. Поле **Кол-во для получения** обновляется, чтобы показать заказанное количество плюс избыточное количество. Следующая процедура описывает, как заполнить поле **Кол-во для получения**.  

1. В заказе на покупку или в документе складской приемки, где полученное количество превышает заказанное, введите фактически полученное количество в поле **Кол-во к получению**.
...
Если учет завершен, учтенные строки покупки удаляются из заказа. При завершении учета появляется соответствующее сообщение. После этого учтенные операции станут доступны на различных страницах, в том числе на страницах **Операции книги поставщиков**, **Операции ГК**, **Операции книги товаров**, **Операции книги ресурсов**, **Приходные накладные** и **Учтенные счета покупки**.
