...
description: Вместо того чтобы учитывать отдельные документы по одному, вы можете выбрать несколько неучтенных документов в списке для пакетного учета немедленно или по плану.
author: SorenGP
...
Также можно проверить на странице **Операции журнала очереди работ**, был ли заказ продажи учтен успешно. Дополнительные сведения см. в разделе [Просмотр состояния или ошибок в очереди работ](admin-job-queues-schedule-tasks.md#to-view-status-or-errors-in-the-job-queue).
