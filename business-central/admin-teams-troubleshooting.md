...
# Устранение неполадок интеграции Microsoft Teams с [!INCLUDE [prod_short](includes/prod_short.md)]
...
## Ошибка "Не удается найти контакт или API сводки контактов" при поиске контактов
...
## Ни одна из моих ссылок не разворачивается в карточку 
...
1. Сначала убедитесь, что приложение [!INCLUDE [prod_short](includes/prod_short.md)] для Teams установлено.
    ...
    Чтобы проверить, войдите в классическое приложение Teams или в Teams в браузере. Затем с левой стороны выберите **Приложения** и найдите **[!INCLUDE [prod_short](includes/prod_short.md)]**. Когда вы найдете приложение **[!INCLUDE [prod_short](includes/prod_short.md)]**, выберите его, чтобы открыть страницу сведений о приложении. Если отображается кнопка **Добавить**, то приложение [!INCLUDE [prod_short](includes/prod_short.md)] не установлено. Узнайте больше в разделе [Установите [!INCLUDE [prod_short](includes/prod_short.md)] приложение для Microsoft Teams](across-install-app-for-teams.md).

    > [!NOTE]
    > Гостевые пользователи не могут сразу устанавливать приложения. Подробнее о сотрудничестве с гостями читайте в разделе [FAQ](teams-faq.md?tabs=collaborating#language). 

2. Затем убедитесь, что вы вошли в систему под правильным идентификатором.

    В Teams перейдите в любой чат и под полем создания сообщения щелкните правой кнопкой мыши значок [!INCLUDE [prod_short](includes/prod_short.md)], затем выберите **Параметры**. В появившемся окне указана учетная запись пользователя, под которой вы вошли в систему. Убедитесь, что это правильная учетная запись пользователя.

3. Убедитесь, что codeunit 2718 **Поставщик сводки по страницам** публикуется как веб-сервис.

    Teams подключается к [!INCLUDE [prod_short](includes/prod_short.md)], используя конечную точку этого codeunit в службе [!INCLUDE [prod_short](includes/prod_short.md)]. Подробнее читайте в статье [Публикация веб-сервиса](across-how-publish-web-service.md).
...
- Например, Teams молча отказывается от попытки отобразить карточку через 15 секунд из-за проблем с сетью.
...
Подробнее об URL-адресах в разделе [!INCLUDE [prod_short](includes/prod_short.md)] URL-адреса [URL-адреса веб-клиента](/dynamics365/business-central/dev-itpro/developer/devenv-web-client-urls) в [!INCLUDE [prod_short](includes/prod_short.md)] Справке для разработчиков и ИТ-специалистов.

## Окно сведений открывается, но перед отображением сведений отображается ошибка

Эта проблема может быть вызвана двумя причинами: отсутствием разрешений в [!INCLUDE [prod_short](includes/prod_short.md)] или параметрами браузера (при использовании Teams в браузере).
...
2. Проверьте настройки своего браузера, если используете Teams в браузере.
    ...
    - Блокировщик всплывающих окон в вашем браузере должен быть отключен или настроен так, чтобы разрешать всплывающие окна из доменов *businesscentral.dynamics.com* или *bc.dynamics.com*. Подробнее о разрешении всплывающих окон читайте в разделе [!INCLUDE [prod_short](includes/prod_short.md)] «Настройка браузера». [...](across-browser-settings.md#popup)
   ...
   Подробнее о минимальных требованиях к браузеру читайте в разделе [Минимальные требования для использования [!INCLUDE [prod_short](includes/prod_short.md)]](product-requirements.md#browsers).

## У меня проблемы с камерой или местоположением в Teams
...
- Для Teams в браузере убедитесь, что настройки браузера разрешают доступ к камере и местоположению для [https://teams.microsoft.com](https://teams.microsoft.com).
...
Подробнее об изменении этих настроек читайте в статье [Моя камера не работает в Teams](https://support.microsoft.com/office/my-camera-isn-t-working-in-teams-9581983b-c6f9-40e3-b0d8-122857972ade?ns=msftteams&version=16&ui=en-us&rs=en-us&ad=us) на сайте поддержки Microsoft.

Приложение [!INCLUDE [prod_short](includes/prod_short.md)] не поддерживает местоположение в классическом приложении Teams. Подробнее читайте в разделе [Часто задаваемые вопросы о командах](teams-faq.md#location).
...
## В Teams отображаются разные языки для моих карточек и сведений карточек
...
- Подробнее об изменении языка в Teams читайте в статье [Изменение настроек в Teams](https://support.microsoft.com/en-us/office/change-settings-in-teams-b506e8f1-1a96-4cf1-8c6b-b6ed4f424bc7) на сайте поддержки Microsoft. 

- Подробнее об изменении языка читайте в разделе [!INCLUDE [prod_short](includes/prod_short.md)] [Изменение основных настроек - Язык](ui-change-basic-settings.md#language).

Узнайте больше о том, как работают языки в Teams, в разделе [!INCLUDE [prod_short](includes/prod_short.md)]  [Часто задаваемые вопросы о Teams](teams-faq.md#language).

## Я отредактировал поле в окне сведений, но мои изменения не были сохранены
...
## На панели средства запуска приложений появилась новая плитка. Как ее удалить?
...
Как администратор, у которого есть разрешения на администрирование Microsoft Entra, вы можете скрыть плитку, выполнив следующие действия:
...
## Повторяющийся текст в окне «Поделиться в Teams»
...
## Невозможно войти в окно «Поделиться в Teams» 
...
## На моих карточках больше нет всплывающей кнопки
...
## Не удается прикрепить карточку к вкладке
...
## Кто-то добавил вкладку, но у меня она не отображается
...
## Другие пользователи видят другую сортировку или компоновку столбцов (не такие, как у автора вкладки)
