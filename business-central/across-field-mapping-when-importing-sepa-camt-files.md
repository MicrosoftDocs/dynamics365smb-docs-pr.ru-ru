...
---
# <a name="field-mapping-when-importing-sepa-camt-files"></a><a name="field-mapping-when-importing-sepa-camt-files"></a><a name="field-mapping-when-importing-sepa-camt-files"></a>Сопоставление полей при импорте файлов SEPA CAMT
...
[!INCLUDE[prod_short](includes/prod_short.md)]поддерживает региональные стандарты SEPA для импорта банковских выписок SEPA (формат CAMT). Дополнительные сведения см. в разделе [Использование расширения AMC Banking 365 Fundamentals](ui-extensions-amc-banking.md).  
 ...
 Стандарт SEPA CAMT имеет локальные вариации. Следовательно, возможно придется изменить универсальное определение обмена данными (представляемое кодом **SEPA CAMT** на странице **Определения обмена данными**), чтобы адаптировать его к локальной вариации стандарта. В следующих таблицах показаны сопоставления элементов полям для таблиц 81, 273 и 274 в реализации SEPA CAMT в [!INCLUDE[prod_short](includes/prod_short.md)].  
...
## <a name="camt-data-mapping-to-fields-in-the-general-journal-table-81"></a><a name="camt-data-mapping-to-fields-in-the-general-journal-table-81"></a><a name="camt-data-mapping-to-fields-in-the-general-journal-table-81"></a>Соответствие данных CAMT полям в таблице "Финансовый журнал" (81)
...
## <a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-table-273"></a><a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-table-273"></a><a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-table-273"></a>Соответствие данных CAMT полям в таблице "Выверка банковского счета" (273)
...
## <a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-line-table-274"></a><a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-line-table-274"></a><a name="camt-data-mapping-to-fields-in-the-bank-acc-reconciliation-line-table-274"></a>Соответствие данных CAMT полям в таблице "Строка выверки банковского счета" (274)
...
|Stmt/Ntry/AddtlNtryInf|AdditionalEntryInformation|Текст|Дополнительные сведения об операции||16|Информация о транзакции|  
 ...
 Элементы в узле **Ntry** , импортируемые в [!INCLUDE[prod_short](includes/prod_short.md)], но не сопоставляемые ни с какими полями, хранятся в таблице **Определение столбца обмена данными**. Пользователи могут просматривать эти элементы на страницах **Журнал выверки платежей**, **Применение платежа** и **Выверка банковского счета**, выбрав действие **Подробные сведения строки выписки с банк. счета**. Дополнительные сведения см. в разделе [Выверка платежей с использованием автоматического применения](receivables-how-reconcile-payments-auto-application.md).

> [!IMPORTANT]
> При импорте банковских выписок CAMT [!INCLUDE[prod_short](includes/prod_short.md)] ожидает, что каждая транзакция будет уникальной, а это означает, что поле **Код транзакции**, которое происходит из тега *Stmt/Ntry/NtryDtls/TxDtls/Refs/EndToEndId* в файле CAMT, должен быть уникальным в рамках открытой выверки банковского счета. Если информации нет, [!INCLUDE[prod_short](includes/prod_short.md)] игнорирует платеж. Если предыдущая банковская выверка на том же банковском счете была разнесена с тем же идентификатором транзакции, что и при текущем импорте, текущая транзакция не будет согласована автоматически, но все равно может быть импортирована.
