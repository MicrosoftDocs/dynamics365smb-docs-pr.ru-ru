---
title: Подготовьтесь к миграции данных клиента с помощью шаблонов
description: Если вы используете RapidStart Services, вы можете использовать шаблоны конфигурации для структурирования существующих данных о клиентах, прежде чем переносить основные данные в новую компанию в Business Central.
author: SorenGP
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
Вы можете создать структуру шаблона и применить ее к любой из таблиц в [!INCLUDE[prod_short](includes/prod_short.md)].  
...
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок введите **Шаблоны конфигураций**, а затем выберите связанную ссылку.  
...
3. В поле **Значение по умолчанию** введите нужное значение. В некоторых случаях, можно использовать значение, которое не является значением, доступным в базе данных. В этом случае можно установить флажок **Пропустить проверку связей**, чтобы разрешить применение информации без ошибок.
    ...
    > [!TIP]  
    > Так как в поле **Значение по умолчанию** не выполняется поиск соответствующих параметров поля [!INCLUDE[prod_short](includes/prod_short.md)], следует скопировать и вставить в шаблон необходимое значение из связанной страницы.
...
Можно создать книгу Excel в качестве шаблона на основе структуры существующей таблицы базы данных. Затем можно использовать шаблон для последовательного сбора данных клиента для последующего импорта в [!INCLUDE[prod_short](includes/prod_short.md)].
...
> [!NOTE]
> Если данные в таблицах в пакете конфигурации содержат даты, например, даты проводки в счетах, даты учитываются в часовом поясе, указанном в [!INCLUDE[prod_short](includes/prod_short.md)]. 
...
4. Нажмите кнопку **ОК**.  
5. Чтобы просмотреть новую карточку товара, выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок введите **Товары**, а затем выберите связанную ссылку.  
...
## <a name="to-use-conversion-templates"></a>Для использования шаблонов преобразования

Вы можете конвертировать контакты в клиентов, поставщиков и сотрудников. 

### <a name="to-convert-a-contact-into-a-customer-vendor-or-employee"></a>Чтобы преобразовать контакт в клиента, поставщика или сотрудника
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") и введите **Контакты**, затем выберите правильный контакт. 
2. На карточке контакта выберите **Действия**, потом **Функции**, затем выберите **Создать в качестве клиента, поставщика, банка или сотрудника**.


## <a name="to-use-a-configuration-template-on-a-record"></a>Использование шаблона конфигурации в записи

Шаблон данных можно применять к любым записям в [!INCLUDE[prod_short](includes/prod_short.md)]. Этот метод можно использовать для изменения или модификации записи. При этом вы перезаписываете существующие значения в записи с помощью этого шаблона. Соответственно, следует соблюдать осторожность при применении шаблона к существующим записям.
...
> [!NOTE]
> Вы не можете использовать команду "Применить шаблон" для очистки полей о клиентах, поставщиках и т. д. Вместо этого нужно использовать функцию **Изменить в Excel**. Дополнительные сведения см. в разделе [Изменение в Excel](across-work-with-excel.md#edit-in-excel).
