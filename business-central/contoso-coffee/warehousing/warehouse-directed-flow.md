---
title: 'Приемка, размещение, перемещение, подбор и отгрузка в расширенной конфигурации склада'
description: Входящие и исходящие процессы могут выполняться разными способами в зависимости от уровня сложности склада.
author: brentholtorf
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
# <a name="walkthrough-of-inbound-and-outbound-flow-in-advanced-warehouse-configuration"></a>Пошаговое руководство. Приход и расход в расширенных конфигурациях склада
...
1. Создание складской отгрузки.  
    ...
    2. Выберите заказ для клиента 10000 для склада БЕЛЫЙ. Внешний номер заказа: *W-1*.
...
2. Определение ячеек для товара для задания места, где он размещается 
    ...
    3.  Выберите действие **Создать**. Добавьте две строки.
    
    |Пункт меню|Код склада|Код ячейки|Устранено|Единице измерения|
    |----------|----------|---------|---|------|  
...
5. Регистрация складского размещения
    ...
    - строка «Поместить», которая перемещает другой МЕШОК в другие фиксированные ячейки (W-05-0002). Это связано с тем, что одна фиксированная ячейка не может вместить полное количество принятого товара.
...
6. Определение ячеек подбора для товара для задания места, откуда он подбирается 
