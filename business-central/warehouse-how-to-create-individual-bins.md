...
## Создание ячейки из карточки склада
...
Производственный центр настроен с кодом ячейки в поле **Код входящей ячейки**. Строки компонента производственного заказа с этим кодом ячейки требуют, чтобы компоненты прямого списания были размещены здесь. Пока компоненты из этой ячейки не израсходованы, другие потребности в компонентах могут приводить к подбору или потреблению из этой ячейки, поскольку компоненты все еще считаются доступным содержимым ячейки. Чтобы содержимое ячейки было доступно только тому требованию на компоненты, которое использует данную ячейку, направляемую в производство, необходимо выбрать поле **Специальная** в строке этого кода ячейки.
...
## Создание отдельных ячеек в журнале создания ячеек
...
2.  В каждой строке заполните поля, которые необходимы для определения имен и характеристик создаваемых ячеек.  
3.  Выберите действие **Создать ячейки**.  

## Для автоматического создания ячеек в журнале создания ячеек

Перед началом автоматического создания ячеек следует определить вид ячеек, необходимых для вашей работы, а также наиболее практически оправданный поток товаров по физической структуре склада.  
...
8. Закончив заполнять поля, нажмите кнопку **ОК**.
...
9. После удаления всех ненужных ячеек выберите действие **Создать ячейки**, и приложение создаст ячейки для каждой строки в журнале.  

Повторяйте процесс для других наборов ячеек, пока не будут созданы все ячейки на складе.  
