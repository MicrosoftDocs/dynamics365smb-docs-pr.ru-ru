...
- Какие затраты несет ваш бизнес.  
...
Места возникновения затрат (или центры затрат) — это подразделения и центры учета прибыли, которые отвечают за затраты и доход. Часто в учете затрат настроено больше мест возникновения затрат, чем в любом измерении, которое настроено в Главной книге. В главной книге обычно используется только первый уровень мест возникновения затрат (центров затрат) для прямых затрат и начальных затрат. В модуле «Учет затрат» создаются дополнительные центры затрат для дополнительных уровней распределения.  

Объекты затрат — это продукты, группы продуктов или услуги, предлагаемые организациями. Эти объекты представляют собой «готовую продукцию», в которой содержатся затраты.  
...
В журналах затрат можно учитывать затраты и действия, которые не поступают из главной книги или не создаются автоматически в результате распределения. Например, можно учитывать чистые операционные расходы, внутренние начисления, распределения и корректирующие операции между типами себестоимости, центрами затрат и объектами затрат по отдельности или на регулярной основе.  

## Распределения затрат

Распределения перемещают затраты и доходы между типами затрат, местами возникновения затрат и объектами затрат. Накладные расходы сначала учитываются в местах возникновения затрат, а затем распределяются по объектам затрат. В качестве примера можно привести отдел продаж, который продает несколько продуктов одновременно. Накладные расходы отдела, такие как заработная плата, расходные материалы и командировочные расходы, первоначально относятся на центр затрат «Продажи». Затем эти затраты распределяются между различными проданными продуктами (объектами затрат) вместе с приобретенными материалами (прямая себестоимость).

Используемая база распределения и точность распределения влияют на результаты распределения затрат. Определение распределения обеспечивает распределение затрат сначала из так называемых предварительных центров возврат по основным центрам затрат, а затем из центров затрат по объектам затрат.  

Каждое распределение состоит из источника распределения и одной или нескольких целей распределения. Вы можете распределять фактические значения или бюджетные значения, используя метод статического распределения на основе конкретного значения. Например, это может быть метраж площади или установленное соотношение распределения 5:2:4. Также можно распределить фактические значения или бюджетные значений с помощью метода динамического распределения с предопределенными 9 базами распределения и 12 динамическими диапазонами дат.  
