...
7. Перейдите к различным страницам и мероприятиям в [!INCLUDE[prod_short](includes/prod_short.md)], к которым требуется разрешить доступ пользователям с этим набором разрешений. Выполните задачи, для которых следует записать разрешения.
