...
- Подробные разрешения, которые вы назначаете в [!INCLUDE[prod_short](includes/prod_short.md)].
  ...
  В этой статье описывается, как вы можете определять, использовать и применять разрешения в [!INCLUDE [prod_short](includes/prod_short.md)], чтобы изменить конфигурацию по умолчанию.  
...
## <a name="to-create-a-permission-set"></a>Создание набора разрешений
...
Техническое обслуживание также проще. Когда вы добавляете системное разрешение, ваш пользовательский набор разрешений будет автоматически обновляться с учетом любых изменений, которые Microsoft вносит в эти разрешения.
...
5. На странице **Набор разрешений** в поле **Тип**, включите или исключите разрешения для объекта следующим образом:
  ...
  > [!NOTE]
  > Самый высокий набор разрешений в иерархии определяет, будет ли разрешение включено или исключено. Если два набора находятся на одном уровне в иерархии, и разрешение включено в один набор, но исключено в другом, разрешение будет исключено.
...
Чтобы полностью исключить добавленный вами набор разрешений, на панели **Результат** выберите строку, выберите **Показать больше параметров**, затем выберите **Исключить**. При исключении набора разрешений на панели **Наборы разрешений** создается строка типа "Исключено". Если вы исключили набор разрешений, но хотите включить его снова, удалите строку на панели **Наборы разрешений**.
...
4. Выберите действие **Разрешения**.
1. На странице **Разрешения** выберите действие **Записать права доступа**, затем выберите действие **Запуск**.  
