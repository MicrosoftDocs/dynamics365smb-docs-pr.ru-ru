...
При открытии нового документа или карты, для которой существует номерная серия, открывается соответствующее окно **Настройка серий номеров**, чтобы можно было настроить серии номеров для этого типа документов или карточек, прежде чем переходить к вводу других данных.

> [!NOTE]  
> Если необходимо включить ручную нумерация, например, для новых карточек товаров, которые были созданы с помощью процесса миграции данных, которые имеют скрытое поле **Номер**. по умолчанию, перейдите в окно **Настройка модуля "Запасы"** и выберите поле **Серия номеров товаров** для открытия и задания для соответствующих серий номеров значения **Ручная нумерация**.
