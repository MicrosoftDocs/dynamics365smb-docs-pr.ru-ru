...
Обменные курсы — это инструмент для расчета стоимости каждой валютной операции в местной валюте (МВ). Страница **Курсы обмена валют**  содержит следующие поля:
...
## <a name="adjust-exchange-rates"></a>Коррекция валютных курсов
...
Для поддержания актуальности валютных курсов можно использовать внешнюю службу.
...
4. Сопоставьте поля из XML-файла с таблицей курсов обмена валют в исходном поле.
5. Примените все необходимые правила преобразования.
6. Чтобы включить службу включите переключатель **Включено**.
...
## <a name="update-currency-exchange-rates-through-a-service"></a>Обновление валютных курсов с использованием службы
...
Время от времени вам может потребоваться исправить ошибку в платежной транзакции, связанную с корректировками прибылей и убытков в связи с конвертацией валют. Вы можете использовать действие **Отменить транзакцию** на страницах **Операции в книге учета банковских счетов**, **Операции в книге учета клиентов** и **Операции в книге учета поставщиков**, чтобы отменить и отменить платежную транзакцию.
