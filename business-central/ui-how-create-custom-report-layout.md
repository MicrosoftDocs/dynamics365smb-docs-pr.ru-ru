...
По умолчанию отчет будет иметь встроенный макет отчета RDLC или Word либо оба типа. Вы не можете изменять встроенные макеты, но можете создавать собственные макеты. Отчет может иметь несколько настраиваемых макетов отчетов, что позволяет переключать макет, который используется по мере необходимости.
