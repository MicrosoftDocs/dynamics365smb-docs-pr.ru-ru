---
title: Сверка платежей клиентов с журналом приходов денежных средств или с записями книги клиентов.
description: Примените денежные поступления или возмещения клиентов к одной или нескольким открытым записям книги клиентов.
author: brentholtorf
ms.author: bholtorf
...
---
# <a name="reconcile-customer-payments-with-a-cash-receipt-journal-or-from-customer-ledger-entries"></a>Сверка платежей клиентов с журналом приходов денежных средств или с записями в книге учета клиентов.

Когда вы получаете платеж наличными от клиента или возвращаете деньги наличными, вы можете применить платеж или возврат для закрытия открытых дебетов или кредитов. Можно задать сумму, которую нужно применить. Например, можно применить частичные платежи к операциям книги клиентов. Закрытие операций в книге клиентов обеспечивает актуальность статистики клиентов, выписок по счетам, финансовых расходов и т. д.
...
* Введите информацию на следующих страницах:
   ...
   * Страница **Кассовый журнал** . Дополнительную информацию см. в разделе  [Чтобы заполнить и разнести журнал приходов денежных средств](#to-fill-and-post-a-cash-receipt-journal).
* Заполнив поле **Примен. к док. - номер** в документах кредит-ноты продажи. Для получения дополнительной информации см.  [Чтобы применить кредитовое авизо к одной записи книги клиентов](#to-apply-a-credit-memo-to-a-single-customer-ledger-entry).
* Используя действие **Установить код применения** для операции книги клиентов. Для получения дополнительной информации см.  [Чтобы применить платеж к одной записи книги клиентов](#to-apply-a-payment-to-a-single-customer-ledger-entry).
...
6. Используйте поле **Номер внешнего документа**, чтобы сохранить идентификатор, например номер чека клиента.
   ...
   > [!NOTE]
   > По умолчанию поле Номер внешнего документа скрыто. Если это так, и вы хотите его использовать, вы можете добавить его с помощью персонализации. Дополнительные сведения см. в разделе [Персонализация рабочей области](ui-personalization-user.md).

7. В поле **Тип счета** выберите **Клиент**
8. В поле  **Номер счета**  выберите клиента.
9. Чтобы разместить заявку одновременно с публикацией журнала, заполните следующие поля:
   1. В поле **Тип баланс. счета** выберите **Счет ГК** для платежей наличными или **Банковский счет** для остальных платежей.
   2. В поле **Номер баланс. счета** выберите кассовый счет для платежей через кассу или соответствующий банковский счет для остальных платежей.
...
1. Выберите значок ![Лампочка, которая открывает функцию «Что вы хотите сделать»](media/ui-search/search_small.png "Что вы хотите сделать"), , введите **Журнал приходных касс** и выберите соответствующую ссылку.
2. Выберите действие **Изменение журнала**.
...
5. Нажмите кнопку **ОК**. На странице  **Кредитовое авизо**  теперь отображается запись, выбранная вами в документе  **Применимо. Введите**  и  **Applies-to Doc. Количество** полей. И сумма учитываемой кредит-ноты, скорректированная для любых возможных скидок оплаты.
6. Учет кредит-ноты.
...
3. Выберите действие  **Записи книги**, а затем выберите строку с записью, которая является применяемой записью.
...
8. Нажмите кнопку **ОК** для учета применения.
    ...
    Если опубликованное заявление привело к закрытым записям книги клиентов, поле  **Открыть**  для этих записей книги очищается.  
...
Приведем пример. Вы применяете запись А в одной валюте к записи Б в другой валюте. Дата проводки в записи A используется для определения обменного курса, который будет использоваться для конвертации сумм в записи B. Обменный курс можно найти на странице **Курсы обмена валют** .  
...
Когда вы исправляете заявку, корректирующие операции создаются и учитываются для всех операций. Корректирующие операции такие же, как и исходные, но имеют противоположный знак в поле **Счет**. Корректирующие операции включают все операции главной книги, полученные из приложения. Например, скидка на оплату и курсовая прибыль/убыток. Записи, закрытые приложением, открываются повторно.  
