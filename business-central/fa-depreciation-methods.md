...
[!INCLUDE [prod_short](includes/prod_short.md)] поддерживает восемь различных методов амортизации основных средств:

* Линейный (SL)
* Уменьшение остатка 1 (DB1)
* Уменьшение остатка 2 (DB2)
* Ум. ост. 1/линейный
* Ум. ост. 2/линейный
...
При линейной амортизации вы амортизируете стоимость актива либо с фиксированным годовым процентом, либо с фиксированной годовой суммой в течение периода амортизации. При использовании линейного метода необходимо определить один из следующих параметров книги амортизации основных средств:  
...
* Сумма амортизации = ((учетная стоимость - восстановленная стоимость) х число дней амортизации) / оставшиеся дни амортизации*  
...
Если вы учтете сумму, которая не соответствует амортизации по методу уменьшаемого остатка 1, например 5000, [!INCLUDE [prod_short](includes/prod_short.md)] предлагает оставшуюся часть ожидаемой суммы.
...
Метод полугодового начисления нормы износа применяется только в том случае, если вы включили переключатель **Использ. начисл. полугод. нормы износа** для основного средства на странице **Карточка основного средства**.  
...
При применении метода начисления полугодовой нормы износа основное средство имеет шестимесячную амортизацию в первый финансовый год вне зависимости от содержимого поля **Дата начала амортизации**.  
...
## <a name="duplicate-entries-to-other-depreciation-books"></a>Дублирование операций в другие книги амортизации

При наличии трех книг амортизации — B1, B2 и B3 — и необходимости копирования операций из B1 в B2 и B3 можно включить переключатель **Часть списка дубликатов** в карточках книг амортизации B2 и B3. Например, использовать этот параметр удобно в следующих случаях:
...
Если вы вводите операцию в книге B1 в журнале основных средств ГК и включаете переключатель **Часть списка дубликатов**, [!INCLUDE [prod_short](includes/prod_short.md)] продублирует операцию в книги B2 и B3 в журнале основных средств при учете этой операции.  
...
Если встроенные методы амортизации не соответствуют вашим потребностям, вы можете определить свой собственный метод амортизации, используя таблицы амортизации. Подробнее о применении определенного пользователем метода амортизации см. в статье [Настройка пользовательского метода амортизации](fa-how-setup-user-defined-depreciation-method.md).
