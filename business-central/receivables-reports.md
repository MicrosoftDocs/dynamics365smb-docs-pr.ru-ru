...
| **Продажи - статистика** |112  |[!INCLUDE [reports-sales-statistics](includes/reports-sales-statistics.md)]<br>Этот отчет также можно использовать для работы с дебиторской задолженностью, так как проще быстро найти разнесенные платежи, скидки и продажи для данного клиента.|
|**Список клиентов**|101| Показывает базовую информацию о клиентах, например учетную группу клиента, группу скидки, пени и данные об оплате, менеджере, стандартной валюте клиента и кредитном лимите в местной валюте (в рублях), а также текущем балансе клиента (в рублях). Отчет может использоваться, например, для ведения информации в таблице "Клиент".|
