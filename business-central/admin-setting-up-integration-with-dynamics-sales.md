...
---
# Настройка учетных записей пользователя для интеграции с Microsoft Dataverse путем синхронизации данных
...
> [!IMPORTANT]
> Во время установки вам будет предложено ввести учетные данные для среды [!INCLUDE[prod_short](includes/cds_long_md.md)]. Укажите учетные данные учетной записи лицензированного пользователя, которому назначена роль безопасности **системного администратора** в среде [!INCLUDE[prod_short](includes/cds_long_md.md)]  и роль [администратора пользователей](/entra/identity/role-based-access-control/permissions-reference#user-administrator) в клиенте, которому принадлежит среда. Этой учетной записи не требуется лицензия на [!INCLUDE[prod_short](includes/prod_short.md)], так как она будет использоваться только для выполнения задач настройки в среде [!INCLUDE[prod_short](includes/cds_long_md.md)].
>
...
Базовое решение интеграции создает для интеграции следующую роль в [!INCLUDE [cds_long_md](includes/cds_long_md.md)]:

* **Интеграция Business Central с Dataverse**: позволяет управлять подключением между [!INCLUDE [prod_short](includes/prod_short.md)] и [!INCLUDE [cds_long_md](includes/cds_long_md.md)]. Обычно она назначается только автоматически созданной учетной записи пользователя для синхронизации.

> [!NOTE]
> Роль **Интеграция Business Central с Dataverse** должен иметь только пользователь приложения, который запускает интеграцию. Пользователю приложения не нужна лицензия на [!INCLUDE [prod_short](includes/prod_short.md)] или [!INCLUDE [cds_long_md](includes/cds_long_md.md)].
