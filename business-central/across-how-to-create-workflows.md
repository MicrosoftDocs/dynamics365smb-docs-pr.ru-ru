...
Если бизнес-сценарий требует событий или откликов рабочего процесса, которые не поддерживаются, партнеру Майкрософт придется создать расширение, в котором реализовано соответствующее событие рабочего процесса.  
...
1) **Событие "когда"** Здесь выбирается триггер.
    Примеры триггера:
...
2) **При условии** **Условия** связаны с событием и открываются для создания фильтров, когда событие запускается
3) **Отклик "то"** **Ответы** отвечает на следующий шаг в работе.
...
10. На экспресс-вкладке **Параметры для выбранного отклика** задайте параметры для отклика рабочего процесса, выбрав значения в различных полях следующим образом:  
    ...
    1. Чтобы указать параметры для рабочего процесса, предусматривающего отправку уведомления, заполните поля, как описано в следующей таблице.  
        ...
        |**Связать целевую страницу**|Задайте другую страницу, чтобы открывалась ссылка в уведомлении вместо страницы по умолчанию. Обратите внимание, что страница должна иметь ту же исходную таблицу, что и соответствующая запись.|
...
4. Чтобы создать первый шаг рабочего процесса, выполните следующие действия.
    ...
    8. Выберите кнопку ОК для возвращения к шагу рабочего процесса.  
...
6. В следующей строке добавьте второй шаг рабочего процесса для события *Запрос на утверждение утвержден*.  
    ...
    4. Нажмите кнопку ОК.  
7. В следующей строке добавьте шаг рабочего процесса для события *Запрос на утверждение отклонен*.  
...
8. В следующей строке добавьте второй шаг рабочего процесса для события *Запрос на утверждение отклонен*.  
    ...
    3. В поле **Отклик "то"** выберите ссылку **Выбрать отклик**, а затем на странице **Отклики рабочего процесса** в поле **Выбрать отклик** отклик *Отправить запрос утверждения для записи и создать уведомление*.  
    4. Нажмите кнопку ОК.  
9. Чтобы включить рабочий процесс, выберите поле **Включено**.  

На следующих рисунках представлен обзор результата этой процедуры.  
