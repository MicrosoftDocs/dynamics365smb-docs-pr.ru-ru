...
## <a name="set-up-allocation-source-and-targets"></a>Настройте источник и цели распределения
...
## <a name="to-set-filters-for-dynamic-allocation-bases"></a>Чтобы установить фильтры для динамических баз распределения
...
### <a name="set-filters"></a>Настройка фильтров
...
|Приобретенные товары (сумма)|№ товара|Да|Да|Да|Учетная группа запасов|

## <a name="scenario-1-defining-static-allocations-based-on-allocation-ratio"></a>Сценарий 1: Определение статических распределений на основе коэффициента распределения
...
В данной статье описывается, как определить три новых целевых объекта распределения затрат для исходного центра затрат PROD с использованием установленного соотношения распределения 5:2:4. Три целевых объекта затрат: ПРИНАДЛЕЖНОСТИ, КРАСКА и ФИТИНГИ.  
...
1. Выберите значок ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок, введите **Распределение затрат**, а затем выберите связанную ссылку.  
2. На странице **Распределение затрат** выберите действие **Создать**.  
3. В поле **ИД** нажмите кнопку <kbd>Ввод</kbd> или введите ИД.  
4. В поле **Уровень** введите **1**.  
5. В полях **Действительно с** и **Действительно по** введите соответствующие даты.  
