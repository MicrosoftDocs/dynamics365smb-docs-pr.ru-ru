...
## <a name="run-tasks-in-the-foreground"></a>Выполнение задач на переднем плане
...
- **Все** — регистрировать пары запрос/отклик для всех транзакций, включая успешные.

> [!NOTE]
> Постоянное ведение журнала ошибок может замедлить работу [!INCLUDE [prod_short](../includes/prod_short.md)]. Чтобы избежать этого, вы можете включить ведение журнала после того, как обнаружите ошибку в синхронизации. Вы можете снова запустить синхронизацию вручную, а затем просмотреть журнал, чтобы выяснить, что пошло не так.

### <a name="manage-log-entry-data"></a>Управление данными операций журнала
...
### <a name="to-review-logs"></a>Чтобы просмотреть журналы:
...
Во избежание негативного влияния на производительность и увеличения размера базы данных возможно, ведение журнала имеет смысл отключить.
...
## <a name="verify-and-enable-permissions-to-make-http-requests-in-a-non-production-environment"></a>Проверка и включение разрешений для выполнения HTTP-запросов в непроизводственной среде
...
### <a name="error-the-sales-header-does-not-exist-identification-fields-and-values-document-typequotenoyour-shopify-store"></a>Ошибка: заголовок продажи не существует. Идентификационные поля и значения: тип документа = "Предложение", № = "ВАШ МАГАЗИН SHOPIFY"
...
### <a name="error-gen-bus-posting-group-must-have-a-value-in-customer-your-shopify-store-it-cannot-be-zero-or-empty"></a>Ошибка: Общая бизнес-группа должна иметь значение в поле "Клиент": "ВАШ МАГАЗИН SHOPIFY". Это значение не может быть нулевым или пустым.
...
### <a name="error-importing-data-to-your-shopify-shop-isnt-enabled-go-to-the-shop-card-to-enable-it"></a>Ошибка: Импорт данных в ваш магазин Shopify не включен. Перейдите в карточку магазина, чтобы включить его
...
### <a name="error-oauth-error-invalid_request-could-not-find-shopify-api-application-with-api_key"></a>Ошибка: Ошибка Oauth, invalid_request: не удалось найти приложение API для Shopify с api_key
...
### <a name="error-internal-error-looks-like-something-went-wrong-on-our-end-request-id-xxxxxxxx-xxxx-xxxx-xxxx-xxxx"></a>Ошибка: внутренняя ошибка. Похоже, с нашей стороны что-то пошло не так. ИД запроса: ХХХХХХХ-ХХХХ-ХХХХ-ХХХХ-ХХХХ
...
### <a name="error-oauth-error-invalid_request-your-account-does-not-have-permission-to-grant-the-requested-access-for-this-app"></a>Ошибка: ошибка Oauth invalid_request: у вашей учетной записи нет разрешения на предоставление запрошенного доступа для этого приложения.

Похоже, что пользователь, запрашивающий доступ, не имеет прав на управление приложениями (возможность управлять и устанавливать приложения и каналы, а также потенциально утверждать плату за приложения). Возможно, вы сможете решить эту проблему, установив приложение от имени владельца учетной записи. Альтернативно вы можете проверить **Разрешения приложения** для пользователя в настройках [**Пользователь и разрешения**](https://www.shopify.com/admin/settings/account) в вашем разделе **Администрирование Shopify**.  

### <a name="messageaccess-denied-for-field-fieldlocationsline0column0pathpathextensionscodeaccess_denieddocumentationhttpsshopifydevapiusageaccess-scopes"></a>[{"message":"Access denied for ИМЯ_ПОЛЯ field.","locations":[{"line":0,"column":0}],"path":["путь"],"extensions":{"code":"ACCESS_DENIED","documentation":https://shopify.dev/api/usage/access-scopes}}]
