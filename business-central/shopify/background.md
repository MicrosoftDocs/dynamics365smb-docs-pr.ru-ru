...
description: Настройка синхронизации данных между Business Central и Shopify в фоновом режиме.
ms.date: 05/26/2024
ms.topic: article
...
> [!NOTE]
> Некоторые элементы могут обновляться несколькими задачами. Например, при импорте заказов, в зависимости от значений параметров на странице **Карточка магазина Shopify** система также может импортировать и обновлять данные о клиентах и/или продуктах. Во избежание конфликтов не забывайте использовать одну и ту же категорию очереди заданий.
>
> Используйте действие **Страница запроса отчета** для определения фильтров. Например, вы можете указать, что импортировать заказы нужно только тогда, когда их статус — **Полностью оплачено**.
...
## <a name="to-check-the-status-of-synchronization"></a>Для проверки состояния синхронизации
