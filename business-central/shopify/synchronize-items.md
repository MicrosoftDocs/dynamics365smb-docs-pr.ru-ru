...
**Товары** в [!INCLUDE[prod_short](../includes/prod_short.md)] эквивалентны **продуктам** в Shopify. Это физические товары, цифровые загрузки, услуги и подарочные карты, которые вы продаете. Существует основные причины синхронизировать товары:

1. Когда вы в первую очередь управляете данными в [!INCLUDE[prod_short](../includes/prod_short.md)]. Вам нужно экспортировать все или некоторые данные оттуда в Shopify и сделать их видимыми. Вы можете экспортировать название товара, описание, изображение, цены, наличие, варианты, информацию о поставщике и штрих-код. После экспорта вы можете просмотреть товары или сразу сделать их видимыми.
...
Прежде чем экспортировать товар в Shopify, соединитель проверяет, существует ли уже товар. Сначала он проверяет, существует ли продукт или вариант со штрих-кодом, поскольку он определен в операции **Ссылки на товары** типа штрих-кода. Если поле **Сопоставление SKU** заполнено, соединитель проверяет, существует ли продукт или вариант с заполненным SKU. Подробнее см. в разделе [Влияние SKU и штрих-кодов продуктов Shopify на сопоставление и создание товаров и вариантов в Business Central](synchronize-items.md#effect-of-shopify-product-skus-and-barcodes-on-mapping-and-creating-items-and-variants-in-business-central).
...
|Название варианта | **Код варианта**.<br>Причина использования **Кода**, а не **Описания** заключается в том, что Shopify требуются уникальные названия вариантов для каждого продукта. В [!INCLUDE[prod_short](../includes/prod_short.md)] **Код** является уникальным, а **Описание** — нет. Неуникальные описания приведут к проблемам при экспорте продуктов.|**Описание** варианта|
...
### Единица измерения в качестве варианта
...
* При работе с матрицей вариантов, например цветом и единицей измерения, если вы хотите импортировать продукты в [!INCLUDE[prod_short](../includes/prod_short.md)], вам следует установить *№ товара + код варианта* в поле **Сопоставление SKU** и убедиться, что поле **SKU** в Shopify имеет одинаковое значение для всех единиц измерения и включает в себя и номер товара, и код варианта.
...
* Вы не сможете добавить параметр «Единица измерения» к существующим продуктам с вариантами (конкретный результат зависит от других настроек, например **Сопоставление SKU**).

### URL-адрес и URL-адрес предварительного просмотра
...
Коннектор может отправлять одну основную цену и одну цену без скидки Shopify. Цены отображаются в полях **Цена** и **Сравнить по цене** на странице Shopify Продукт (Shopify Вариант).

В следующей таблице описаны настройки, которые можно использовать для управления процессом определения и экспорта цен.

|Поле|Описанием|
|------|-----------|
|**Ценовая группа клиентов**|Определите цену товара в Shopify. Берется продажная цена этой ценовой группы клиентов. Если группа не указана, используется цена в карточке товара. Коннектор не будет использовать ценовую группу клиента.|
|**Скидочная группа клиентов**|Определите скидку, которая будет использоваться при расчете цены товара в Shopify. Цены со скидкой сохраняются в поле **Цена**, а полная цена хранится в поле **Сравнить по цене**. Коннектор не будет использовать группу скидок клиента от клиента.|
...
### Синхронизация цен для B2B

[!INCLUDE [shopify-preview](../includes/shopify-preview.md)]

Если вы используете Shopify B2B, вы можете настроить коннектор для синхронизации цен для Shopify каталогов, связанных с клиентами B2B.

#### Синхронизировать каталоги из Shopify

1. Выберите значок ![Лампочка, которая открывает функцию Что вы хотите сделать.](../media/ui-search/search_small.png "Что вы хотите сделать"), введите **Каталоги Shopify**, а затем выберите соответствующую ссылку.
2. Выберите **Получить каталоги**.

Вы можете получить доступ только к каталогам, связанным с компаниями B2B. Для получения дополнительной информации см. раздел [Компании B2B](synchronize-customers.md#b2b-companies). Обратите внимание, что каталоги не содержат продукцию. Управлять содержимым каталога можно в разделе Shopify Администрирование.

#### Синхронизировать цены для B2B-каталога

1. Выберите значок ![Лампочка, которая открывает функцию Что вы хотите сделать.](../media/ui-search/search_small.png "Что вы хотите сделать"), введите **Каталоги Shopify**, а затем выберите соответствующую ссылку.
2. Выберите запись, для которой вы хотите определить и экспортировать цены.
3. Используйте доступные настройки для настройки порядка определения цен. Настройки аналогичны тем, которые используются для синхронизации полей  **Цена**  и  **Сравнить по цене**  в Shopify Товаре (Shopify Варианте).
4. Включите переключатель **Синхронизировать цены**.
5. Выберите **Синхронизировать цены** и дождитесь завершения синхронизации цен.
...
1. Перейдите к поисковой ![лампочке, которая открывает функцию Tell Me.](../media/ui-search/search_small.png "Что вы хотите сделать") введите **Магазины Shopify** и выберите соответствующую ссылку.
2. Выберите магазин, для которого вы хотите синхронизировать запасы, чтобы открыть страницу **Карточка магазина Shopify**.
3. Выберите действие **Синхронизация запасов**.

### Синхронизация запасов со страницы продукции Shopify

1. Перейдите к поисковой ![лампочке, которая открывает функцию Tell Me.](../media/ui-search/search_small.png "Что вы хотите сделать") введите **Продукты Shopify** и выберите соответствующую ссылку.
...
####  Пример расчета свободных запасов (не зарезервированных)

Имеется 10 единиц товара А в наличии и три невыполненных заказа на продажу. Один заказ с количеством *1* зарезервирован из записи книги товаров, один с количеством *2* не зарезервирован и один с количеством *3* зарезервировано из заказа на поставку. Для этого метода дата синхронизации не важна.

|Значение, используемое для обновления уровня запасов|Комментарий|
|-----------------|-----------------|
|9|Запасы «10» за вычетом заказа на продажу с зарезервированными запасами из записи в книге товаров. Другие заказы на продажу игнорируются.|

### Два подхода к управлению выполнением заказов
...
#### Пример использования переключателя «Местоположение продукта по умолчанию»
