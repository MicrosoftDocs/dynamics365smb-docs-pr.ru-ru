...
|**Разделитель полей SKU**|Используется, когда параметр **Сопоставление SKU** установлен в значение **[№ товара + код варианта](synchronize-items.md#effect-of-shopify-product-skus-and-barcodes-on-mapping-and-creating-items-and-variants-in-business-central)**.<br>Определите разделитель, который будет использоваться для разделения SKU.<br>Таким образом, если в Shopify вы создаете вариант с SKU "1000/001", вы вводите "/" в поле **Разделитель полей SKU**, чтобы создать номенклатурный номер в [!INCLUDE[prod_short](../includes/prod_short.md)] как "1000", а код варианта товара как "001".|
...
|**Код шаблона клиента**|Выберите шаблон по умолчанию, который будет использоваться при расчете цены. Подробнее см. в разделе [Настройка налогов](setup-taxes.md).|
...
|Описание|Объединяет расширенные тексты и атрибуты, если соответствующие переключатели активированы в карточке магазина Shopify. Учитывается код языка.|Не используется.|
...
* Для расчета цены важно, чтобы было указано значение в поле **Стандартный шаблон клиента**. Подробнее см. в разделе [Настройка налогов](setup-taxes.md).
* Вводить **код валюты** нужно только в случае, если в вашем интернет-магазине используется валюта, отличная от локальной валюты (МВ). Для указанной валюты должен быть настроен обменный курс. Если ваш интернет-магазин использует ту же валюту, что и [!INCLUDE[prod_short](../includes/prod_short.md)], оставьте поле пустым.
