...
|**ЕИ в качестве варианта**| Выберите этот вариант, если вы хотите, чтобы все единицы измерения товара экспортировались как отдельные варианты. Добавьте поле с помощью персонализации. Подробнее см. в разделе [Единица измерения в качестве варианта](synchronize-items.md#unit-of-measure-as-variant).|
|**Имя варианта для ЕИ**| Используйте это поле вместе с **ЕИ в качестве варианта**, чтобы указать, под каким параметром добавлять варианты, представляющие единицы измерения. Значение по умолчанию — *Единица измерения*. Добавьте поле с помощью персонализации.|
...
|**ЕИ в качестве варианта**| Выберите этот вариант, если вы хотите, чтобы некоторые параметры экспортировались и импортировались в виде единиц измерения, а не как варианты. Добавьте поле с помощью персонализации. Подробнее см. в разделе [Единица измерения в качестве варианта](synchronize-items.md#unit-of-measure-as-variant).|
|**Имя варианта для ЕИ**| Используйте это поле вместе с **ЕИ в качестве варианта**, чтобы указать, какой параметр содержит варианты, представляющие единицы измерения. Значение по умолчанию — *Единица измерения*. Добавьте поле с помощью персонализации.|

> [!NOTE]
> Если вы хотите экспортировать много товаров и вариантов, некоторые из них могут быть заблокированы. Включить заблокированные товары и варианты в расчет цен невозможно, поэтому они не экспортируются. Соединитель пропускает эти товары и варианты, поэтому фильтровать их на странице запроса **Добавить товар в Shopify** не нужно.
...
* Соединитель отправляет запрос на обновление цен в Shopify, если цена в [!INCLUDE[prod_short](../includes/prod_short.md)] изменилась. Например, если вы синхронизировали товары и цены, а затем изменили цену в Shopify, выбор действия **Синхронизировать цены в Shopify** не окажет никакого влияния на цену в Shopify, поскольку новая цена, рассчитанная соединителем, совпадает с ценой, хранящейся в варианте Shopify из предыдущей синхронизации. Значение поля **Цена до скидки** обновляется только в случае обновления основной цены. 
...
Если логистика обрабатывается [!INCLUDE[prod_short](../includes/prod_short.md)], где вас может быть любое количество складов, представляющих распределительные центры, вы не создаете склады в Shopify; соединитель Shopify автоматически создает службы выполнения Business Central, и вы можете привязать запасы (с помощью фильтров складов) с нескольких складов к одной записи служб выполнения. В результате в Shopify нет информации о том, откуда отправляется товар, есть только информация об отслеживании. Находясь в [!INCLUDE[prod_short](../includes/prod_short.md)], вы можете выбирать запасы по наличию и близости к пункту назначения. 
...
|Служба выполнения Business Central и главный| Ошибка: стандартные местонахождения Shopify нельзя использовать в сочетании с местонахождениями служб выполнения|
