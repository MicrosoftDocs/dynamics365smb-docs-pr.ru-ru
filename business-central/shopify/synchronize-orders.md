...
ms.locfileid: "9076309"
---
# <a name="synchronize-and-fulfill-sales-orders"></a>Синхронизация и выполнение заказов на продажу

В этой статье описаны необходимые настройки и действия, которые необходимо выполнить для синхронизации и выполнения заказов на продажу от Shopify в [!INCLUDE[prod_short](../includes/prod_short.md)].
...
После завершения импорта вы можете изучить заказ Shopify и найти всю связанную информацию. Например, найдите платежные операции, стоимость доставки, уровень риска или выполнение, если заказ уже был выполнен в Shopify. Вы также можете увидеть любое подтверждение заказа, отправленное клиенту, выбрав действие **Страница статуса Shopify**.
...
Если переключатель **Автоматическое создание заказов** включен в **карточке магазина Shopify**, [!INCLUDE[prod_short](../includes/prod_short.md)] пытается создать торговый документ после импорта заказа. Если в процессе возникают проблемы, например отсутствие клиента или продукта, вам необходимо устранить проблему. Затем вы можете попытаться снова создать заказ на продажу.
...
В то время как импортированный заказ Shopify содержит информацию о налогах, налоги пересчитываются при создании документа продажи. Этот пересчет делает важным, чтобы настройки НДС (налогов) были правильными в [!INCLUDE[prod_short](../includes/prod_short.md)].
...
- Цена с учетом налога. Поле **Цены с учетом налога**/**Цены с учетом НДС** в созданном торговом документе зависит не от покупателя, а от **Шаблона клиента** с карточки магазина Shopify или шаблона клиента для каждой страны.

### <a name="impact-of-edits-of-orders"></a>Влияние правок заказов

|Изменить|Воздействие|
|------|-----------|
|В Shopify измените место выполнения | Исходное местоположение будет синхронизировано с [!INCLUDE[prod_short](../includes/prod_short.md)]. |
|В Shopify отредактируйте заказ и измените количество| Заголовок заказа и дополнительные таблицы будут обновлены в [!INCLUDE[prod_short](../includes/prod_short.md)], строки не будут. |
|В Shopify отредактируйте заказ и добавьте новый товар | Заголовок заказа будет обновлен, строки — нет. |
|В [!INCLUDE[prod_short](../includes/prod_short.md)] измените местоположение на другое местоположение, сопоставленное с местоположениями Shopify. Учет отгрузки. | После синхронизации выполнения местоположение будет обновлено в Shopify. |
|В [!INCLUDE[prod_short](../includes/prod_short.md)] измените местоположение на другое местоположение, не сопоставленное с местоположениями Shopify. Учет отгрузки. | Выполнение не будет синхронизировано с Shopify. |
|В [!INCLUDE[prod_short](../includes/prod_short.md)] уменьшите количество. Учет отгрузки. | Заказ Shopify будет отмечен как частично выполненный. |
|В [!INCLUDE[prod_short](../includes/prod_short.md)] добавьте новый товар. Учет отгрузки. | Заказ Shopify будет отмечен как выполненный. Строки не будут обновляться. |
...
Платежные операции, совершенные в Shopify, синхронизируются с заказами и могут быть просмотрены со страницы *Заказы Shopify*.
