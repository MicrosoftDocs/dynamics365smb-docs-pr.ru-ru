...
Например, вы можете создать демонстрационную учетную запись или использовать пробную версию. Узнайте больше в разделе [Подготовка демонстраций [!INCLUDE[prod_short](../includes/prod_short.md)]](/dynamics365/business-central/dev-itpro/administration/demo-environment.md) и [Регистрация в пробной версии](../trial-signup.md). 
...
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](../media/ui-search/search_small.png "Что вы хотите сделать") значок, введите **Магазин Shopify**, а затем выберите связанную ссылку.
...
1. Включите переключатель **Журнал включен**.
2. Выключите переключатель **Разрешить фоновые синхронизации**.
3. Выберите **В Shopify** в поле **Синхронизировать товар**.
4. Выберите **В Shopify** в поле **Синхронизировать изображения товара**.
5. Включите переключатель **Синхронизировать атрибуты товара**.
6. Включите переключатель **Отслеживание запасов**.
7. Выберите **Отклонить** в поле **Политика запасов по умолчанию** .
8. Включите переключатель **Автоматическое создание неизвестных клиентов**.
9. Заполните поле **Код шаблона клиента** соответствующим шаблоном.
10. Заполните **Счет стоимости доставки**, **Счет чаевых** счетом доходов. Например, в США используйте `40100`.
11. Включите переключатель **Автоматическое создание заказов**.

Настройка сопоставлений складов:

1. Выберите действие **Места**, чтобы открыть **Адреса магазинов Shopify**.
2. Выберите действие **Получить места Shopify** для импорта всех местоположений, определенных в Shopify.
3. В **Фильтре по складу** введите `''|EAST|MAIN`.
4. Деактивируйте переключатель **Отключить**, чтобы включить синхронизацию запасов для выбранного склада Shopify.
...
В **Интернет-магазине Shopify** откройте каталог товаров. Примечание.
...
8. В поле **Кредитная карта** введите `1`, если вы используете *(для тестирования) Bogus Gateway*. Если вы используете *Shopify Payments* в тестовом режиме, введите `5555 5555 5555 4444` в поле **Кредитная карта**.
...
1. Выберите значок ![Лампочка, которая открывает функцию «Что вы хотите сделать»](../media/ui-search/search_small.png "Что вы хотите сделать"), введите **магазин Shopify** и выберите соответствующую ссылку.
...
5. Выберите товар **1896-S, стол ATHENS**.
...
7. Выберите **Расширенный текст** и создайте новый расширенный текст, действительный для всех языковых кодов. В поле **Описание** введите `Shopify`. 
8. Добавьте следующее описание с HTML-тегами: `<b>Simple stylish design</b> blends with any ensemble. <i>Available in two editions.</i>`.
9. Выберите **Цены продажи** и добавьте новые цены, как показано в следующей таблице:
...
10. Выберите **Торговые скидки** и добавьте новую скидку:
...
11. Выберите **Ссылки на товары** и добавьте следующие строки:
...
12. Закройте **Карточку товара**.
13. Выберите товар **1920-S, стол для конференций ANTWERP**.
14. Выберите **Настроить запасы** и в поле **Новые запасы** введите `100` для местоположений *ВОСТОК* и *ЗАПАД*. 
1. Выберите **ОК**.
1. Закройте **Карточку товара**.
...
5. Установите фильтр *ТАБЛИЦА* в поле **Код категории товара**.
6. Выберите действие **Синхронизировать изображения продуктов**.
7. Выберите действие **Синхронизация запасов**.
...
1. Перейдите на бесплатную 30-дневную пробную версию без примеров данных. Дополнительную информацию см. в разделе [Добавление собственных данных в пустую пробную версию](/dynamics365/business-central/dev-itpro/administration/trials-subscriptions.md#add-your-own-data-to-an-empty-trial-company).
2. Выберите значок ![Лампочка, которая открывает функцию «Что вы хотите сделать»](../media/ui-search/search_small.png "Что вы хотите сделать"), введите **Магазин Shopify**, а затем выберите связанную ссылку.
...
7. Включите переключатель **Журнал включен**.
8. Выключите переключатель **Разрешить фоновые синхронизации**.
9. Выберите **Из Shopify** в поле **Синхронизировать товар**.
5. Включите переключатель **Автоматически создавать неизвестные товары**.
11. Заполните поле **Код шаблона товара** соответствующим шаблоном.
12. Выберите **Из Shopify** в поле **Синхронизировать изображения товара**.
13. Выберите **Все клиенты** в **Импорт клиентов из Shopify**
14. Включите переключатель **Автоматическое создание неизвестных клиентов**.
15. Заполните поле **Код шаблона клиента** соответствующим шаблоном.
16. Заполните **Счет стоимости доставки**, **Счет чаевых** счетом доходов. Например, в США используйте `40100`.
17. Включите переключатель **Автоматическое создание заказов**.
