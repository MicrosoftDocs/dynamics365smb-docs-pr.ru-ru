...
description: 'Узнайте, как подавать отчетность о торговле с организациями из других стран ЕС с использованием системы Интрастат.'
author: altotovi
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
ms.author: altotovi
---
# <a name="work-with-intrastat-reporting"></a><a name="work-with-intrastat-reporting"></a><a name="work-with-intrastat-reporting"></a>Работа с отчетностью Интрастат
...
## <a name="fill-in-the-intrastat-report"></a><a name="fill-in-the-intrastat-report"></a><a name="fill-in-the-intrastat-report"></a>Заполнение отчета Интрастат
...
4. В поле **Статистический период** укажите месяц, за который подается отчетность. Вводите период в виде четырехзначного числа без пробелов и символов. В зависимости от страны введите сначала месяц, а затем год, или наоборот. Например, для июня 2022 года введите *2206* или *0622*.
...
7. В конечном итоге вы можете настроить дополнительные параметры на экспресс-вкладке **Дополнительно**:
   ...
   3. **Показать записи товарных издержек**, чтобы указать, должны ли отображаться прямые затраты, которые ваша организация назначила и учла как товарные издержки.
   4. **Пропускать операции, по которые не выставлены счета**, чтобы указать, должны ли исключаться из процесса операции книги товаров, которые отгружены или получены, но по которым еще не выставлены счета.
...
## <a name="modify-the-intrastat-report"></a><a name="modify-the-intrastat-report"></a><a name="modify-the-intrastat-report"></a>Изменение отчета Интрастат
...
## <a name="validate-intrastat-lines"></a><a name="validate-intrastat-lines"></a><a name="validate-intrastat-lines"></a>Проверка строк Интрастат
...
Запустите отчет **Контрольный список отчетов Интрастат**, чтобы проверить строки Интрастат, прежде чем они будут экспортированы в требуемый формат. Проверка выполняется внутри **отчета Интрастат**.

## <a name="recalculating-weight-or-supplementary-unit-of-measure"></a><a name="recalculating-weight-or-supplementary-unit-of-measure"></a><a name="recalculating-weight-or-supplementary-unit-of-measure"></a>Пересчет веса или дополнительной единицы измерения

Если вы видите сообщение об ошибке *"Общий вес" в строке отчета Интрастат не должен быть пустым*, это скорее всего связано с тем, что вы не задали поле **Вес нетто** в используемом источнике, товаре или основнмо средстве. В этом случае найдите карточку товара или основного средства и добавьте необходимое значение. После этого необходимо просто заново открыть **отчет Интрастат** и выполнить следующие действия:
...
## <a name="intrastat-rules"></a><a name="intrastat-rules"></a><a name="intrastat-rules"></a>Правила Интрастат

### <a name="grouping-lines"></a><a name="grouping-lines"></a><a name="grouping-lines"></a>Группирование строк
...
### <a name="fixed-assets-reporting"></a><a name="fixed-assets-reporting"></a><a name="fixed-assets-reporting"></a>Отчеты по основным средствам
...
### <a name="intrastat-report-statuses"></a><a name="intrastat-report-statuses"></a><a name="intrastat-report-statuses"></a>Статусы отчетов Интрастат
