...
Если вы уже интегрировали [!INCLUDE[crm_md](includes/crm_md.md)] с [!INCLUDE[prod_short](includes/prod_short.md)], вы можете продолжить синхронизацию данных, используя свои настройки. Однако, если вы обновите или выключите интеграцию [!INCLUDE[crm_md](includes/crm_md.md)], чтобы включить его снова, вы должны подключиться через [!INCLUDE[prod_short](includes/cds_long_md.md)]. Дополнительные сведения см. в разделе [Обновление интеграции с Dynamics 365 Sales](admin-upgrade-sales-to-cds.md).
...
> [!TIP]
> Если вы также хотите интегрироваться с  Microsoft Dynamics 365 Field Service, в руководстве предусмотрен дополнительный шаг, который может помочь. Чтобы узнать больше об интеграции с Field Service, перейдите к разделу  [Интеграция с Microsoft Dynamics 365 Field Service](admin-integrate-field-service.md).
...
При настройке интеграции есть параметры, которые управляют направлением синхронизации заказов на продажу и способом их отправки.

Параметр **Двунаправленная синхронизация заказов на продажу** позволяет синхронизировать заказы на продажу из Sales в [!INCLUDE [prod_short](includes/prod_short.md)] и наоборот. Например, если покупатель передумает о продукте или количестве в  [!INCLUDE[crm_md](includes/crm_md.md)], вы можете заархивировать торговый документ и создать новый документ в [!INCLUDE[prod_short](includes/prod_short.md)]. То же самое относится и к изменениям в [!INCLUDE[prod_short](includes/prod_short.md)]. Например, при изменении цен, сумм налогов или ожидаемых дат отгрузки эти изменения передаются в [!INCLUDE[crm_md](includes/crm_md.md)]. Двунаправленная синхронизация помогает держать ваших продавцов в курсе последних изменений и статуса заказов на продажу.
...
Вы можете создать несколько единиц измерения для каждой группы единиц и назначить группы для продуктов в [!INCLUDE[crm_md](includes/crm_md.md)]. После этого вы можете синхронизировать продукты с элементами и ресурсами в [!INCLUDE[prod_short](includes/prod_short.md)]. Вы можете вручную связать единицы измерения товаров или единицы измерения ресурса с группой единиц. Если вы это сделаете, если группа единиц для элемента или ресурса не связана с группой единиц в  [!INCLUDE[crm_md](includes/crm_md.md)], например, потому что группа единиц не существует,  [!INCLUDE[prod_short](includes/prod_short.md)] автоматически создает группу объектов в [!INCLUDE[crm_md](includes/crm_md.md)].
...
2. Поскольку новые продукты создаются в  [!INCLUDE[crm_md](includes/crm_md.md)]  с новыми группами единиц, во избежание дублирования названий выполните один из следующих шагов:
...
|Товаров|При синхронизации товаров с [!INCLUDE[crm_md](includes/crm_md.md)] продуктами [!INCLUDE[prod_short](includes/prod_short.md)] автоматически создается прайс-лист в [!INCLUDE[crm_md](includes/crm_md.md)]. Чтобы избежать ошибок синхронизации, не следует изменять данный прайс-лист вручную.|
...
|Заказы на продажу|Когда вы включаете интеграцию заказов на продажу, при выпуске заказов на продажу в  [!INCLUDE[prod_short](includes/prod_short.md)] , которые были созданы на основе отправленных заказов на продажу в  [!INCLUDE[crm_md](includes/crm_md.md)] , они синхронизируются с заказами на продажу в [!INCLUDE[crm_md](includes/crm_md.md)]. Прежде чем синхронизировать заказы, мы рекомендуем сначала синхронизировать все таблицы, связанные с заказом. Например, продавцы и прайс-листы. Поле кода менеджера по продажам в заголовке заказа определяет владельца связанной таблицы в [!INCLUDE[crm_md](includes/crm_md.md)].|
...
## Подключение к локальным версиям Business Central выпуска 2019, волна 1, и Microsoft Dynamics NAV 2018
 ...
 Microsoft Power Platform команда [объявила](/power-platform/important-changes-coming#deprecation-of-office365-authentication-type-and-organizationserviceproxy-class-for-connecting-to-dataverse) об отказе от использования  Office 365 типа аутентификации. Если вы используете версию [!INCLUDE[prod_short](includes/prod_short.md)] локальной версии, более раннюю, чем [!INCLUDE [prod_short](includes/prod_short.md)] выпуск волны 1 2019 г., для подключения к  [!INCLUDE[crm_md](includes/crm_md.md)]  необходимо использовать тип аутентификации OAuth. В сети. Шаги в этом разделе описывают, как подключить следующие версии продуктов:
...
* Вы должны иметь подписку Microsoft Azure. Пробный аккаунт работает для регистрации приложения.
