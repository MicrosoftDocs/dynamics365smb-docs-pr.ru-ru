---
title: Как зарезервировать товары
...
# <a name="reserve-items"></a>Резервные товары
...
4. На вкладке **Строки**  в группе **Функции**  выберите действие **Резервировать** .
5. На странице **Резервирование**  выберите строку продаж, строку заказа, затем выберите действие **Зарезервировать из текущей строки** .  
...
## <a name="reserve-items-in-bulk"></a>Массовое резервирование товаров
...
2. Выберите действие **Получить спрос** . Откроется страница **Получить спрос на резерв** .
1. На странице **Получить спрос для резервирования**  укажите тип спроса, который вы хотите зарезервировать из доступного инвентаря.
1. Заполните соответствующим образом поля фильтров. [!INCLUDE [tooltip-inline-tip_md](includes/tooltip-inline-tip_md.md)]
1. (Необязательно) Чтобы сразу распределить товары, выберите действие **Распределить**.
1. На странице **Политика распределения** выберите политику для каждого шага.

   |Политика распределения  |Описанием  |
   |---------|---------|
   |Базовое (без конфликтов)     | Распределяет запас в адрес источника спроса, если нет конфликтов и спрос может быть полностью покрыт. Например, у вас есть заказ на продажу с количеством 10 штук и работа с количеством 7 штук. Если у вас есть 20 штук на складе, оба источника спроса получают полное количество. Если у вас на складе 12 штук, запасы не распределяются. Вам нужно будет распределить количество вручную.        |
   ...
   |По приоритету клиента|Распределение по полю **Приоритет** на странице **Карточка клиента**. В случае низкого количества запасов Business Central в первую очередь снабжает клиентов с более высоким приоритетом.|
...
2. На странице **Бронирование** выберите действие **Записи бронирования** на быстрой вкладке **Строки** .  
...
Такая практика называется конкретным резервированием, поскольку вы резервируете из того количества товара X, которое относится к лоту X. Напротив, если вы резервируете только из того количества товара X, то это просто обычное, неконкретное резервирование. Подробнее см. в разделе [Сведения о проектировании — трассировка и резервирование товара](design-details-item-tracking-and-reservations.md).
