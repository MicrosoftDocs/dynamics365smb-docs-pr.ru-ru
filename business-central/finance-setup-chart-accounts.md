...
Новое поле, **Блокировать удаление счетов ГК**, добавлено на страницу **Настройка главной книги**. Поле действует как дополнительная проверка, когда пользователь пытается удалить счет, в котором есть операции книги после даты, указанной в поле **Проверять удаление счета ГК с операциями после**.

Когда поле **Блокировать удаление счетов ГК** задано как *Да*, вы не можете удалить счета ГК, которые имеют записи книги после даты в поле **Проверять удаление счета ГК с операциями после**. Для удаления такой учетной записи пользователь, имеющий доступ к странице **Настройка главной книги** должен сначала установить это поле как *Нет*. Затем счет можно удалить.  

Мы рекомендуем установить поле **Блокировать удаление счетов ГК** как *Да*. Мы также рекомендуем всегда устанавливать дату в поле **Проверять удаление счета ГК с операциями после**, например время, необходимое для хранения финансовых данных.  
