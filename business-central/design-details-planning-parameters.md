...
description: В этой теме описаны различные параметры планирования, которые можно использовать, и их влияние на систему планирования.
author: SorenGP
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
> [!NOTE]  
>  При расчете горизонтов планирования система планирования игнорирует все рабочие календари, определенные в поле **Код базового календаря** на страницах **Информация об организации** и **Карточка склада**.  

Страховой запас времени по умолчанию на странице **Производство - настройка** должен быть настроен хотя бы на 1 день. Может быть известна дата оплаты по спросу, но не время оплаты. Планирование составляет расписание в обратном направлении, чтобы удовлетворить общий спрос, и если время подготовки страхового запаса не определено, товары могут прибыть слишком поздно, чтобы удовлетворить спрос.  

Три дополнительных поля периода повторного заказа — **Период перепланирования**, **Период накопления лота** и **Буферный период** — также играют роль в определении времени повторного заказа. Для получения дополнительных сведений см. [Оптимизация сроков и объемов дозаказа](design-details-planning-parameters.md#optimize-when-and-how-much-to-reorder).  
...
5. Если поле **Горизонт планирования** не заполнено, будет добавлен только полный спрос на ту же дату срока оплаты.  
     ...
     Следующие поля периода повторного заказа также играют роль в определении количества для повторного заказа: **Период перепланирования**, **Период накопления лота** и **Буферный период**. Для получения дополнительных сведений см. [Оптимизация сроков и объемов дозаказа](design-details-planning-parameters.md#optimize-when-and-how-much-to-reorder).  
...
## <a name="use-low-level-codes-to-manage-derived-demand"></a>Использование кодов низкого уровня для управления производным спросом

Используйте низкоуровневые коды, чтобы производный спрос на компоненты продвигался до нижних уровней спецификации. Более подробное объяснение этого см. в разделе [Приоритет/код нижнего уровня товара](design-details-central-concepts-of-the-planning-system.md#item-priority--low-level-code).
...
> [!IMPORTANT]
> Если не выбрано поле **Динамический код нижнего уровня**, следует запустить пакетное задание **Вычислить код нижнего уровня** до расчета плана поставки (пакетное задание **Вычислить план**).  

> [!NOTE]
> Даже если выбрано поле **Динамич. код нижнего уровня**, коды нижнего уровня компонентов не изменяются динамически, если родительская спецификация удаляется или становится несертифицированной. Это может затруднить добавление новых элементов в конце структуры продукта, так как может быть превышено максимальное количество кодов нижнего уровня. Таким образом, для крупных структур продуктов, которые достигают ограничения на количество кодов нижнего уровня, рекомендуется часто выполнять пакетное задание **Вычислить код нижнего уровня**, чтобы сохранить структуру.  

### <a name="optimize-low-level-code-calculation"></a>Оптимизация вычисления кода нижнего уровня

Выберите поле **Оптимизировать вычисление кода нижнего уровня**, чтобы указать, что вы хотите использовать новый, более быстрый метод вычисления кода нижнего уровня. Обратите внимание, что новый расчет выполняется иначе, и его использование может привести к нарушению работы расширений, которые полагаются на существующий метод. Новый метод расчета заменит текущий в будущих версиях.
