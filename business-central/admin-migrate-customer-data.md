...
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок введите **Пакет конфигурации**, а затем выберите связанную ссылку.  
...
При применении данных, импортированных из Excel или пакета RapidStart, [!INCLUDE[prod_short](includes/prod_short.md)] рассматривает и обрабатывает сопоставление на основе отношений таблиц:  

- Если соответствие определяется непосредственно для поля в таблице, [!INCLUDE[prod_short](includes/prod_short.md)] использует его.  

- Если поле связано с другой таблицей, [!INCLUDE[prod_short](includes/prod_short.md)] выполняет поиск соответствия, определенного для первичного ключа в связанной таблице. Связанная таблица, однако, должна быть частью пакета конфигурации.  

- Если сведения соответствия определены в обоих местах: непосредственно для поля и для первичного ключа в связанной таблице, то [!INCLUDE[prod_short](includes/prod_short.md)] будет выполнять поиск соответствия в обоих местах.  
...
В следующем примере показано, как [!INCLUDE[prod_short](includes/prod_short.md)] реализует определения соответствия.  
...
## <a name="to-add-additional-values-to-prod_short"></a>Добавление дополнительных значений в [!INCLUDE[prod_short](includes/prod_short.md)]  
1. Выберите ![Лампочка, которая открывает функцию Что вы хотите сделать.](media/ui-search/search_small.png "Что вы хотите сделать") значок введите **Пакеты конфигураций**, а затем выберите связанную ссылку.  
2. Выберите таблицу, в которую требуется добавить дополнительные значения, затем, на вкладке **Таблицы** выберите действие **Поля**.  
3. Для полей, для которых требуется, чтобы приложение [!INCLUDE[prod_short](includes/prod_short.md)] разрешило дополнительные значения во время миграции, установите флажок **Создать отсутствующие коды**.  
...
> [!IMPORTANT]  
> Не изменяйте столбцы в таблицах Excel. Если они перемещаются, изменяются или удаляются, то импортировать лист в [!INCLUDE[prod_short](includes/prod_short.md)] невозможно.
...
Теперь можно приступать к импорту файлов данных миграции в [!INCLUDE[prod_short](includes/prod_short.md)], которые содержат унаследованные данные клиента.

## <a name="to-import-customer-data"></a>Импорт данных клиента
После ввода данных клиента в файлы миграции данных в Excel вам следует импортировать файлы в [!INCLUDE[prod_short](includes/prod_short.md)].
...
Перед применением записей к базе данных [!INCLUDE[prod_short](includes/prod_short.md)] необходимо проверить данные клиентов.  
...
После импорта всех записей миграции данных, которые являются допустимым и не имеют ошибок, можно применить записи к базе данных [!INCLUDE[prod_short](includes/prod_short.md)].  
