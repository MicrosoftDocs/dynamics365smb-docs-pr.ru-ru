---
title: Настройка отправки текстовых сообщений на счет сопоставление для повторяющихся платежей
...
---
# <a name="map-text-on-recurring-payments-to-accounts-for-automatic-reconciliation"></a>Текст карты повторяющихся платежей по счетам для автоматической сверки
...
Аналогичная функция существует для выверки избыточных суммы в строках журнала выверки платежей на произвольной основе. Для получения дополнительной информации см. раздел [Сверка платежей, которые не могут быть применены автоматически](receivables-how-reconcile-payments-cannot-apply-auto.md).

Платежи, учитываемые на основе сопоставления текст-счет, не применяются к открытым операциям, но учитываются на заданных счетах помимо создания операций журнала банковского счета. Соответственно, функция отправки текстовых сообщений на счет сопоставление подходит для повторяющихся денежных поступлений или расходов, таких как частые покупки автомобильного топлива или банковские сборы и проценты, которые регулярно отражаются в банковской выписке и не требуют соответствующего делового документа. Более подробную информацию см. в разделе «Пример — отправка текстового сообщения на счет сопоставление для оплаты расходов на топливо» в этой статье.
...
## <a name="example-text-to-account-mapping-for-bank-fees"></a>Пример: Текстовое сообщение на счет сопоставление для оплаты банковских сборов
