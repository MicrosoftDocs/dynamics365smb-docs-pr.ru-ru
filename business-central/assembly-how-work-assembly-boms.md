...
Заказы на сборку используются для изготовления готовых товаров из компонентов в рамках процесса, который может быть выполнен с помощью одного или нескольких простых ресурсов, не являющихся машинными или производственными центрами, или вовсе без ресурсов. Например, процесс сборки может заключаться в том, чтобы взять две бутылки вина и один пакет кофе, а затем упаковать их как подарочный набор.  

Сборочная спецификация — это основные данные, которые определяют, какие товары-компоненты попадают в собранный конечный товар и какие ресурсы используются для сборки сборочного элемента. Когда вы вводите сборочный элемент и количество в заказе на сборку, строки заказа на сборку заполняются в соответствии со сборочной спецификацией. Заказ содержит по одной строке заказа на сборку для каждого компонента или ресурса. Подробнее см. в статье [Управление сборкой](assembly-assemble-items.md).

[!INCLUDE[prod_short](includes/prod_short.md)] также поддерживает производственные спецификации. Производственные спецификации отличаются от сборочных спецификаций тем, что включают более сложные процедуры, включая использование ресурсов, производственную маршрутизацию и производственные или машинные центры. Подробнее о различиях см. в статьях [Работа со спецификациямиl](inventory-how-work-BOMs.md) и [Создание производственных спецификаций](production-how-to-create-production-boms.md).

## Создание сборочной спецификации

Для определения товара, который состоит из других товаров и, возможно, ресурсов, необходимых для сборки товара, следует создать сборочную спецификацию.  
...
> [!TIP]
> Сборочные элементы могут иметь варианты — точно так же, как любые другие товары; это помогает сократить ваш список продуктов. Подробнее об этой функции см. в статье [Управление вариантами продукта](inventory-item-variants.md).

## Редактирование сборочных спецификаций

Вы можете редактировать строки в сборочной спецификации в любое время. Однако спецификация может использоваться в текущих продажах или сборках родительского товара. Изменение спецификации может повлиять на эти действия. Выберите действие **Применимость**, чтобы увидеть, в каких товарах она используется, и могут ли быть затронуты заказы на продажу или сборку.
...
Из любого документа покупки и продажи, который содержит сборочный элемент, можно использовать специальное действие для замены строки для сборочного элемента новыми строками для его компонентов. Это действие удобно использовать, например, если вы хотите продавать компоненты как комплект, который представляет сборочный элемент.
...
> [!CAUTION]  
> После использования действия **Раскрыть спецификацию** простых способов отменить его не существует. Необходимо удалить строки заказа на продажу, которые представляют компоненты, а затем снова ввести строку заказа на продажу для сборочного элемента.
...
1. Выберите значок ![Лампочка, которая открывает функцию «Что вы хотите сделать»](media/ui-search/search_small.png "Что вы хотите сделать"), введите **Товары**, а затем выберите соответствующую ссылку.
2. Откройте карточку сборочного элемента. (Поле **Сборочная спецификация** на странице **Товары** содержит значение **Да**).
3. На странице **Карточка товара** выберите действие **Сборочная спецификация**.
...
|**Все уровни**|Вычисляет стандартную себестоимость сборочного товара как сумму следующих значений:</br></br>* Расчетная себестоимость всех базовых сборочных спецификаций в сборочной спецификации.</br>* Себестоимость всех приобретаемых товаров в сборочной спецификации.|
