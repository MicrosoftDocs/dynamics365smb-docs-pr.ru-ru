...
1. Выберите значок ![Лампочка, которая открывает функцию Что вы хотите сделать](media/ui-search/search_small.png "Что вы хотите сделать"), введите **Выверка банковского счета**, затем выберите связанную ссылку.
...
2. В окне **Перемещ. банк. выверки в фин. журнал** укажите, какую главную книгу требуется использовать, затем нажмите кнопку **ОК**.
...
4. Чтобы проверить результат работы перед его учетом, выберите действие **Тестовый отчет**. Откроется отчет **Выписка по банковскому счету**, содержащий те же поля, что и в заголовке окна **Выверка банковского счета**.
