...
Производственная спецификация содержит основные данные, которые описывают компоненты и узлы, используемые в производстве товара. Когда вы создаете производственный заказ на товар, его производственная спецификация определяет расчет потребностей в материалах, как представлено на странице **Компоненты производственного заказа**.

[!INCLUDE[prod_short](includes/prod_short.md)] также поддерживает сборочные спецификации. Заказы на сборку используются для изготовления готовых товаров из компонентов в рамках простого процесса, который может быть выполнен с помощью одного или нескольких простых ресурсов, не являющихся машинными или производственными центрами. Также это может быть процесс, выполняемый без ресурсов вовсе. Например, процесс сборки может заключаться в том, чтобы взять две бутылки вина и один пакет кофе, а затем упаковать их как подарочный набор. Дополнительные сведения см. в разделе [Сборочные или производственные спецификации](inventory-how-work-boms.md#assembly-boms-or-production-boms).  
...
8. В поле **Брак (%)** можно ввести фиксированный процент компонентов, отбракованных в процессе производства. После того как эти компоненты будут готовы к использованию в запущенном производственном заказе, этот процент добавляется к ожидаемому количеству в поле **Количество потребления** производственного журнала. Дополнительные сведения см. в разделе [Регистрация потребления и выхода продукции](production-how-to-register-consumption-and-output.md).  
...
Например, новые версии спецификаций используются, например, когда товар заменяется другим, или когда клиент требует специальный вариант продукта. Принцип версии позволяет управлять различными версиями производственных спецификаций. Структура версии производственной спецификации соответствует структуре производственной спецификации. Основной разницей является время действия версий. Действительность определяется по дате начала.  
...
Столбцы «Формула расчета», «Длина», «Ширина», «Высота» и «Вес» не отображаются, поскольку они используются только некоторыми пользователями. Если требуется использовать вычисление количества, необходимо сначала вывести на отображение эти колонки.  

Формула расчета определяет отношение между отдельными компонентами. В качестве формулы расчета можно использовать следующие варианты:  
...
В спецификации требуется 70 металлических деталей со следующими размерами: длина = 0,20 м и ширина = 0,15 м. Введите значения следующим образом: Формула расчета = Длина x Ширина, Длина = 20, Ширина = 15, Кол-во в = 70.

Кол-во в x Длина * Ширина, таким образом, Кол-во = 70 x 0,20 м x 0,15 м = 2,1 м2 дает количество.  
