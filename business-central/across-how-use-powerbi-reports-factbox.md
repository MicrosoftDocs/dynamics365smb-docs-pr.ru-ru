...
2. Выберите **Получить данные** и начните выбирать источник данных для отчета.
    ...
    На этом шаге вы указываете страницы списка Business Central, которые содержат данные, которые вы хотите включить в отчет. Например, чтобы создать отчет для списка продаж, убедитесь, что набор данных содержит информацию, связанную с продажами.
...
6. Сохраните отчет и назовите его.
    ...
    Важно задать отчету имя, содержащее имя страницы списка, связанной с отчетом. Например, если отчет предназначен для страницы списка **Товары**, включите слово *товары* где-то в названии.  
...
8. Протестируйте отчет.
    ...
    После того как отчеты были опубликованы в вашей рабочей области, они должны быть доступны из информационной панели Power BI на странице списка в [!INCLUDE[prod_short](includes/prod_short.md)].
...
В этом разделе приведены инструкции по устранению проблем, с которыми вы можете столкнуться при попытке просмотреть отчет Power BI для страницы списка в [!INCLUDE[prod_short](includes/prod_short.md)].  
...
Вероятно, это потому, что в названии отчета нет названия страницы списка, которая отображается. Очистите фильтр, чтобы получить полный список доступных отчетов Power BI.  
