...
2.  Выберите действие **Расчет пополнения ячеек**.  
    ...
    [!INCLUDE[d365fin](includes/d365fin_md.md)] создает строки, которые точно указывают, каким образом товары из ячеек с более низким рейтингом перемещаются в ячейки с более высоким рейтингом.  
